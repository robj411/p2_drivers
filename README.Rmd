...
In setting up a country, we sample values for $\tilde{M}$ (from which we get $`M(\textbf{1})`$). At the same time, we sample the proportion of contacts that come from workplaces, and workplace-related contacts. From these, we get $M^{\text{WW}}(\textbf{1})$ and $M^{\text{CW}}(\textbf{1})$, constructing the matrices and normalising. 
