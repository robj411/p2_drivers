...
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm]
    ...
    \draw [arrow] (S) -- node [above] {$k_{1,v}(t)$} (E1);
...
The rate of infection of susceptible individuals, $k_{1,v}(t)$, is defined as
...
\node[latent] (S) {$S_{v=0}$} ; %
    ...
    \draw [arrow] (S02) edge[bend right] node [above,yshift=.5cm] {$k_{12}$} (em);
    ...
    \draw [arrow] (S) edge[bend right] node [left] {$w_{0,2}$} (S02);
...
Matrix U is diagonal and $U_{ii}(\textbf{1})=0$ for $i>N$ [@Haw2020]. Consumer-to-worker contacts (matrix $V$) describe contacts experienced by workers from consumers per sector. Note that $V_{ij}(\textbf{1})=0$ for $i>N$. Matrix $\hat{V}(\textbf{1})$ is the complement of matrix $V(\textbf{1})$, computed by multiplying through by population, transposing, and dividing again by population.
...
Likewise, $T^{(T)}(\textbf{1})$ is also sampled as a fraction of total contacts. $T_{ij}^{(T)}(\textbf{1})\geq 0$ for $i=1,...,N$. $T_{ij}^{(T)}(\textbf{1})=0$ for $i>N$.
...
Here, there is linear scaling of $V_{ij}(\textbf{1})$ with respect to working from home, and linear scaling with respect to sector closure, which becomes superlinear scaling for sectors as individuals are moved out of the compartment, as with matrix $U(x)$.
