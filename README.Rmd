...
```{tikz vaccinetransitions, fig.cap = "Vaccine state transitions. $S$: susceptible. $S^{(V)}$: vaccinated but not yet protected. $R$: recovered. $v$: vaccination status.", fig.ext = 'png',echo=F}
\usetikzlibrary{shapes}
\usetikzlibrary{fit}
\usetikzlibrary{arrows}
\usetikzlibrary{positioning,arrows.meta}
...
\node[latent] (S) {$S_{v=0}$} ; %
    ...
    \node[latent, below=of S] (S01) {$S_{0,1}^{(V)}$} ; %
    \node[latent, below=of S01] (S1) {$S_{v=1}$} ; %
    \node[latent, draw=white,right=of S1] (emv) {$...$} ; %
    \node[latent, right=of emv] (Rv) {$R_{v=1}$} ; %
    \node[latent, below=of S1,yshift=-1cm] (S12) {$S_{1,2}^{(V)}$} ; %
    \node[latent, left=of S12,xshift=.7cm] (S02) {$S_{0,2}^{(V)}$} ; %
