...
This document describes the daedalus model that is used in the vaccine-impact application (url to follow). The daedalus model simulates a single epidemic in a single country. Details of how the daedalus model is used as a part of the methodology of the vaccine-impact application is presented in a separate report, which also details the scenarios which are expressed as vaccination rates and are inputs to the daedalus model.

(NB. If equations are not legible, try (a) refreshing, (b) a different browser, (c) the pdf.)
...
```{r}
eqtext = 'l_g^{\\text{(life)}} = \\frac{\\sum_{a\\in g}N_a\\tilde{l}_a}{\\sum_{a\\in g}\\tilde{N}_a}; '
if (knitr::is_html_output()) {
  cat(paste0("```math\n",eqtext,"\n```",collapse=''))
} else {
  cat(paste0("$$\n",eqtext,"\n$$",collapse=''))
...
```{r}
eqtext = 'l_g^{\\text{(death)}} = \\frac{\\sum_{a\\in g}N_a\\tilde{l}_aP(D|I,a)}{\\sum_{a\\in g}N_aP(D|I,a)}; '
if (knitr::is_html_output()) {
  cat(paste0("```math\n",eqtext,"\n```",collapse=''))
} else {
  cat(paste0("$$\n",eqtext,"\n$$",collapse=''))
...
```{r}
eqtext = '\\text{VSL}=\\text{VSL}_{\\text{USA}}\\left(r_p\\frac{\\text{GDP}}{\\text{GDP}_{\\text{USA}}}\\right)^{r_e} '
if (knitr::is_html_output()) {
  cat(paste0("```math\n",eqtext,"\n```",collapse=''))
} else {
  cat(paste0("$$\n",eqtext,"\n$$",collapse=''))
...
We approach this differently from @Doohan2025. Instead of contact matrices from @Prem2021, we use those from @Walker2020. Instead of work contacts from @Beraud2015, we use those from @Jarvis2024. @Haw2020 modelled closures using a combination of moving workers between sector compartments and a non-working compartment, and scaling of contacts. Here, we only use contacts to model closures, and do not move workers out of their compartments. An advantage of this is that workers within sectors retain their infection histories. 
...
```{r conagefrac,fig.cap='Distribution of non-school and non-work contacts made in hospitality settings by age group, from (ref:jv).', echo=F,message=F,warning=F, out.width="50%"}
...
```{r pairs, fig.cap="Correlations between tourism-related data. First: (ref:unkey). Second to fourth: (ref:unint). Fifth to seventh: OECD.",fig.height=7, fig.align="center",echo=F,warning=F,message=F}
...
We sample pathogen profiles by defining distributions over the pathogen parameters. The distributions are made using data sourced by @Doohan2025 (shown in Table \@ref(tab:pathogenprofile)), and are described in Table \@ref(tab:pathogenparameters). Age profiles for severity ratios are shown in Figure \@ref(fig:ratesbyage). We sample parameter values from distributions informed by the seven pathogen profiles. $\text{R}_0$ is truncated at 1.5 and 3.5 following @whittakerQuantifyingImpactBroadly2024. 
...
- Population distribution by age [@Doohan2025]
- Life expectancy [@Doohan2025]
- Number of workers per sector [@Doohan2025]
- GVA per worker per sector [@Doohan2025]
- Community contact matrix [@Walker2020]
- Testing rate [@Doohan2025]
...
ggplot(subset(hmaxi,!is.na(igroup)&igroup!='')) + 
  ...
  facet_wrap(~igroup) +
  theme_bw(base_size = 15) +
  labs(x='Second doses administered per day, % population',y='Number of countries')
...
```{r tvradio,echo=F,warning=F,message=F,include=F,eval=F}
