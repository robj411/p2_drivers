...
ggplot(plotscens) + 
  ...
  theme(legend.position = 'top')
sixty <- setDT(do.call(rbind,lapply(tvaxlist,function(x)x[c(which.min(abs(x$Percent-60)),nrow(x)),])))
sixty[,t50:=min(Day),by=Scenario]
sixty[,coverage:=max(Percent),by=Scenario]
ggplot(unique(sixty[,.(Scenario,t50,coverage,col)])) + 
