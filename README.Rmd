...
```{r,fig.cap='All results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/dominance_BAU1.png")
```

```{r,fig.cap='BPSV results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/fig1bpsv_BAU1.png")
```

```{r,fig.cap='R&D results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/fig2rnd_BAU1.png")
```

```{r,fig.cap='Capacity reservation results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/fig3capres_BAU1.png")
```

```{r,fig.cap='Combinations results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/fig4comb_BAU1.png")
```

```{r,fig.cap='Equity results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/fig5eq_BAU1.png")
```

```{r,fig.cap='All results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/dominance_BAU2.png")
```

```{r,fig.cap='BPSV results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/fig1bpsv_BAU2.png")
```

```{r,fig.cap='R&D results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/fig2rnd_BAU2.png")
```

```{r,fig.cap='Capacity reservation results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/fig3capres_BAU2.png")
```

```{r,fig.cap='Combinations results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/fig4comb_BAU2.png")
```

```{r,fig.cap='Equity results.',echo=F,warning=F,message=F, out.width="50%"}
knitr::include_graphics(path = "cepi_results/fig5eq_BAU2.png")
```

```{r,message=F,echo=F,warning=F,fig.cap='Vaccine coverage over time by income level.'}
# tab2 <- readxl::read_xlsx('data/20250522 pandemic delivery scenarios - BAU2.xlsx')
tab <- readxl::read_xlsx('data/20250520 pandemic delivery scenarios - BAU1.xlsx')
...
#     for(il in 1:3){
#       scen1index = (scen-1)*6 + (il-1)*2 + 1
#       scen2index = (scen-1)*6 + (il-1)*2 + 1
#       comp1 = df_list2[[scen1index]]$Percent
#       comp2 = df_list2[[scen1index+1]]$Percent
#       old1 = df_list[[scen2index]]$Percent
#       old2 = df_list[[scen2index+1]]$Percent
#       difference = difference + sum(abs(comp1-old1)+abs(comp2-old2))
#     }
#     if(difference>1e-10)  print(c(scen))
# }
...
ggplot(subset(plotscens,Scenario%in%c('BAU1',scenario_names[c(1,19,20)]))) + 
...
```{r,message=F,echo=F,warning=F,fig.cap='Total SSV coverage over time.'}
popsizes <- c(718255072+3398187527, 2503136362, 1240629858)
popsizes = popsizes/sum(popsizes)
tvaxlist <- list()
# group scenarios by colour
  colourlists <- lapply(list(bpsv = c(1,2,3,13:15,28:31), rnd = c(4,5,16:18), 
                      capres = c(6:9,20:27),
                      del = c(10:11), eq = c(12,19)),function(x) just_scen_names[x])
  colourlists$bau = bau_names
  getsixty = matrix(0,nrow=nScen,ncol=2)
    # if(difference<1e-10)  print(c(scen2, scen))
...
for(scen in 1:nScen){
  ...
  for(il in 1:3){
    ...
    # comp1 = df_list[[scen1index]]$Percent
    old1 = df_list[[scen2index]]$Percent
    tvax = tvax + rev(popsizes)[il]*(old1)
    if (il==1) # HIC
      getsixty[scen,1] = which.min(abs(old1-60))
    if (il==3) # LLMIC
      getsixty[scen,2] = which.min(abs(old1-60))
    # print(c(scen,scen1index,scen2index))
  }
  tvaxlist[[scen]] = data.frame(Day=1:length(tvax),Scenario=scenario_names[scen],Percent=tvax,
                                col=names(colourlists)[sapply(colourlists,function(x)scenario_names[scen]%in%x)])
}

plotscens <- do.call(rbind,tvaxlist)
cscheme = 'plasma'
ggplot(plotscens) + 
  geom_line(aes(x=Day,y=Percent,colour=col,group=Scenario),linewidth=1.5,alpha=.75,show.legend=F) +
...
```{r t50vscoverage,fig.cap="Time taken to get to 60% coverage with SSV second doses for HIC vs LLMIC.",message=F,echo=F,warning=F}



getsixty = as.data.frame(getsixty)
getsixty$Scenario = scenario_names
getsixty$col = sapply(scenario_names,function(scen) names(colourlists)[sapply(colourlists,function(x)scen%in%x)])
ggplot(getsixty) +
  geom_abline(intercept=0,slope=1,col='grey',linewidth=2) + 
  ggrepel::geom_label_repel(aes(x=V1,y=V2,label=Scenario,fill=col,
  ...
  theme(legend.position = 'top') +
  labs(x='HIC days to 60% coverage',y='LLMIC days to 60% coverage')  +
  annotate(geom='text',x=900,y=900,angle=34,label='Line of equality',vjust = 1.3,colour='darkgrey',size=6) +
  coord_fixed(2/3)
