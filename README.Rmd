...
```
  ...
  is the rate of recovery of hospitalised patients, where $`p_{D}^{j}(t)=\hat{p}_{D}^{j}f_H(t)`$ is the baseline probability to die given hospitalisation, adjusted by a factor encoding the increase in fatality rate as hospital occupancy increases, $`f_H(t)=\max\{1,1+1.87(H(t)-H_{\text{max}})/H_{\text{max}}\}`$. $T_{H}^j(t) = p_D^j(t)T_{H:D} + (1-p_{D}^{j}(t))T_{H:R}$ is the expected time to be in compartment $H$: $T_{H:D}$ is the expected duration before death and $T_{H:R}$ is the expected duration before recovery. Finally,
