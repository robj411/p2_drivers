...
for discount rate $r=0.03$, number $N_a$ students currently age $a$, and expected number of years of work $n=45$. $p_{13}$ is mean annual earnings, $p_{14}$ is the extent and amount of time (in years) schools are closed: 
$$p_{14}=\frac{1}{365}\int_{t=1}^{T}(1-x_{ed}(t))dt,$$
$p_{15}=0.08$ is the rate of return for one year, $S$ is the total number of students, $p_{16}$ is the proportion of students affected, and $\int_t I_S(t)dt$ represents education lost due to student sickness with COVID-19. $p_1$ is compliance with the requirement to self isolate if infectious and $p_2$ is the fraction of cases identified through testing.
