...
$T_{H}^j(t) = p_D^j(t)T_{H:D} + (1-p_{D}^{j}(t))T_{H:R}$ is the expected
time to be in compartment $H$: $T_{H:D}$ is the expected duration before
death and $T_{H:R}$ is the expected duration before recovery. Finally,
