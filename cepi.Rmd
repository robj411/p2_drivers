...
Numbers of deaths are translated into discounted years of life lost (dYLL). This takes into account the life expectancy of each person who dies. dYLLs are valued using the value of a statistical life, which we estimate as being 160 times GDP per capita [@Robinson2019].
...
for(vl in 1:length(vaccination_levels)){
  ...
  for(bl in 1:length(bpsv_levels)){
    ...
    colnames(tabres) <- c('Policy','Income group','Total costs','dYLLs','Education','GDP')
...
ggplot(meltmeantab) + 
      geom_bar(aes(x=factor(Policy,levels=c('No Closures','School Closures','Economic Closures','Elimination')),
                   y=value,fill=factor(variable,levels=c('education','deaths','gdp'),labels=c('Education','dYLLs','GDP')),
...
vioplot$variable <- factor(vioplot$variable,
                           levels=c('Cost','dYLLs','School','GDP_loss'),
                           labels=c('Total','dYLLs','Education','GDP'))
...
colnames(tabres) <- c('BPSV','Vaccine','Income group','Total value','dYLLs','Education','GDP','Scenario')


write.csv(tabres,'resultsdiff.csv',row.names = F)

tabres <- tabres[,.(Scenario,`Income group`,`Total value`,dYLLs,Education,GDP)]
