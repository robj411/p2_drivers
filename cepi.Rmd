---
title: "Project Proposal: CEPI stage 2; DAEDALUS"
...
The objective of the proposed work is to use scenario modelling to ascertain the value of the development and distribution of a vaccine 100 days after identification of a new SARS virus (SARS-X) with pandemic potential. In addition, we will estimate the value of a broadly protective SARS vaccine (BPSV) that would be available prior to the development of the SARS-X--specific vaccine. We will estimate the values in monetary terms, combining economic losses due to business closures and lost education, and lives lost monetised using the value of a statistical life (VSL).

We simulate hypothetical pandemics caused by a SARS virus with a disease profile informed by past epidemics. We use the integrated epi-econ model DAEDALUS developed in our group for estimating the benefit of pandemic preparedness (P2) [@Haw2020]. We construct three representative hypothetical countries using real-world demographic, societal and economic data from 197 countries. We evaluate outcomes for four mitigation strategies ("unmitigated", "adaptive economic closures", "school closures", and "elimination") for each vaccination scenario, selecting the cost-minimising strategy for each country type. We estimate the value of the programme to each country type as the difference in societal costs between scenarios. 

We will provide some summary assessments of the impacts of the parameters on outcomes using
suitable methods, for example, value-of-information methods. We will account for uncertainty in as
many parameters as possible. Parameter distributions will be based on data for actual countries and
assumed to be independent of each other.

Our modelling rests on severe simpliﬁcations and many assumptions. We will assess impacts over
two years. Each country is modelled independently of all others and demand and supply shocks from interconnected
economies can only be considered in very rudimentary way, if at all.
...
We consider three stylised "countries" and parametrise models using data from constituent countries according to the World Bank income-classification categories:
 ...
 - UMIC, using inputs from UMICs
 - HIC, using inputs from HICs
 
## Vaccination scenarios

Vaccination scenarios are encoded via the parameters for vaccine effects, the time that vaccination begins, the rate of vaccine administration, and uptake among the population. The baseline scenario mimics vaccination as observed in the COVID-19 pandemic, with all parameters taken from observations in 2020 and 2021. The 100-day vaccine scenario assumes vaccine-effect profiles similar to those of mRNA COVID-19 vaccines, and that vaccination begins 100 days after SARS-X is sequenced, with administration rate and uptake TBC. The BPSV scenario assumes the 100-day vaccine events will occur, and that, prior to that, the BPSV will be available, with effect parameters more similar to those of an influenza vaccine, with vaccination beginning at the time of identification of the SARS-X virus, and administration rate and uptake TBC.

## The model

We use the integrated economic--epidemiological model DAEDALUS [@Haw2020]. The epidemiological component of the model consists of seven disease states (susceptible, exposed, asymptomatic infectious, symptomatic infectious, hospitalised, recovered, and died), in triplicate to represent vaccination states. The population is stratified by age (into four age groups: infants, adolescents, working-age adults, and retirement-age adults). The working-age adults are further stratified by sector of work (into 46 groups: 45 sectors according to the XX classification, and one non-working group). We will use epidemiological parameters expected of a future SARS pandemic, where we use past pandemics to inform our expectations. Epidemiologically relevant demographic parameters, such as mixing matrices, will be drawn from published estimates [@Beraud2015; @Prem2021; @Walker2020].

The economic model takes as input the GVA per sector, which uses the OECD classification of 45 economic sectors. Both models take as input the economic configuration over time: this is the extent to which each sector is open in each period. A sector is open to a maximum amount of 100%, when its full monthly GVA value is realised. If a sector is open to 50%, it contributes 50% of its maximum GVA for that period. In the epidemic model, contacts associated with that sector -- between workers, from workers to consumers, and between consumers -- are likewise scaled down.
...
Taken together, these form the societal cost (SC) of the epidemic in the country, expressed as a percentage of GDP (for the year prior to the pandemic -- here, 2019). For each scenario and each country type, we choose the mitigation strategy that minimises the expected SC - call it the minimum expected SC, or MESC. The value of the 100-day vaccine in each country type is the MESC. of the 100-day vaccine scenario minus the MESC of the baseline scenario. Similarly, the value of the BPSV in each country type is the MESC of the BPSV scenario minus the MESC of the 100-day vaccine scenario.
...
difflist <- c()
for(vl in 2:length(vaccination_levels)){
  for(ct in 1:length(countrytype_levels)){
    vaccination_level <- vaccination_levels[vl]
    countrytype <- countrytype_levels[ct]
    difftab <- as.data.frame(readRDS(paste0('code/results/difftab_',countrytype,'_',vaccination_level,'.Rds')))
    nvar <- ncol(difftab) - 1
    for(i in (nvar-3):nvar){
      difftab[,i] = difftab[,i]/difftab$GDP*100;
    }
    difftab$Country <- countrytype
    difftab$Vaccine <- vaccination_level
    difftab$CountryVaccine <- paste0(countrytype,', ',vaccination_level)
...
vioplot <- melt(difflist[,colnames(difflist)%in%c('Cost','Deaths','School','GDP_loss','igroup','CountryVaccine')],id.vars=c('igroup','CountryVaccine'))
...
ggplot(vioplot,aes(fill=igroup, y=value, x=igroup)) + 
  ...
  facet_grid(CountryVaccine~variable, scales="free") + 
...
meantab <- setDT(difflist)[,.(deaths=mean(Deaths),
                        education=mean(School),
                        gdp=mean(GDP_loss)),by=.(igroup,Country,Vaccine)]
...
tabres <- setDT(difflist)[,list(costs=paste0(round(mean(Cost),1),' (',paste0(round(quantile(Cost,c(0.025,0.975)),1),collapse=', '),')'),
                        ...
                        ),by=.(Country,Vaccine,igroup)]
colnames(tabres) <- c('Country','Vaccination','Income group','Total costs','YLLs','Education','GDP')

write.csv(tabres,'resultsdiff.csv',row.names = F)
lg <- 'Value of vaccines: expected difference between BAU and 100 days, and between 100 days and BPSV. Mean values and 95% prediction intervals. \\label{tab:tabresdiff}'
...
for (i in 1:length(income_levels)){
    for (k in 1:length(strategies)){
        ...
        results <- read.csv(paste0('code/results/VOI_',inp3,'_',income_level,'_BAU_Origin.csv'),header=T);
...
```{r results100,echo=F,warning=F,message=F,results='asis'}

meltmeantab <- c()
allallresults <- data.frame()
p <- list()
  ...
  for(ct in 1:length(countrytype_levels)){
    ...
    for (i in 1:length(income_levels)){
        for (k in 1:length(strategies)){
            inp3 <- strategies[k];
            income_level <- income_levels[i];
            results <- read.csv(paste0('code/results/VOI_',inp3,'_',income_level,'_',vaccination_level,'_',countrytype,'.csv'),header=T);
            results$strategy <- inp3
            results$igroup <- income_level
            allresults <- rbind(allresults,results)
        }
    ...
    p[[vl]][[ct]] <- ggplot(vioplot,aes(fill=igroup, y=value, x=igroup)) + 
      geom_violin(position="dodge", alpha=0.5, outlier.colour="transparent") +
    ...
    allresults$countrytype <- countrytype
    ...
    meltmean <- reshape2::melt(meantab)
    meltmean$scencountry <- paste0(vaccination_level,', ',countrytype)
    ...
    # write.csv(tabres,paste0('results_',vaccination_level,'_',countrytype,'.csv'),row.names = F)
    lg <- paste0('Results for the ',vaccination_level,' scenario in a ',countrytype,' country. Mean values and 95% prediction intervals. \\label{tab:tabres100}')
    if (!knitr::is_html_output(excludes = "markdown")) { 
...
```{r BAUviolinspill,fig.cap='Model results for the BAU scenario in Origin countries.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}
p[[1]][[1]]
```

```{r BAUviolinsecond,fig.cap='Model results for the BAU scenario in secondary countries.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}
p[[1]][[2]]
```

```{r violin100spill,fig.cap='Model results for the 100-day scenario in Origin countries.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}
p[[2]][[1]]
```

```{r violin100second,fig.cap='Model results for the 100-day scenario in secondary countries.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}
p[[2]][[2]]
```

```{r importtime,fig.cap='Hospital occupancy at time of response.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}
ggplot(subset(allallresults,strategy=='No Closures'&vaccination_level=='BAU'),aes(x=Hospital_occupancy_at_response,y=..density..)) +
  ...
  facet_wrap(~factor(countrytype,levels=countrytype_levels),scales='free') +
  theme_bw(base_size = 15) +
...
```{r iseq,fig.cap='Closures for HICs under the elimination strategy.',echo=F,warning=F,message=F,fig.height=5,fig.width=7}
## iseq ########################

iseq <- setDT(read.csv('code/results/iseq.csv'))
...
ggplot(iseq) +
  ...
  facet_wrap(~factor(`Country type`,levels=1:2,labels=countrytype_levels)) +
...
```{r responsetime,fig.cap='Secondary countries: importation time vs. response time. Where importation time is before response time, the epidemic unfolds as in the Origin country, but with an earlier response time.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}
ggplot(subset(allallresults,strategy=='No Closures'&vaccination_level=='BAU'&countrytype=='Secondary'),aes(x=Importation_time,y=Response_time)) +
  # geom_histogram(colour='white',fill='white') +
  geom_point(aes(shape=Importation_time>Response_time),size=3,show.legend=F) +
  # facet_wrap(~factor(countrytype,levels=countrytype_levels),scales='free') +
  theme_bw(base_size = 15) +
...
```{r barBAUOrigin,fig.cap='Expected values of model results.',echo=F,warning=F,message=F,fig.height=7.5,fig.width=6}

meltmeantab$scencountry <- factor((meltmeantab$scencountry), levels=unique(meltmeantab$scencountry))
ggplot(meltmeantab) + 
      geom_bar(aes(x=factor(strategy,levels=c('No Closures','School Closures','Economic Closures','Elimination')),y=value,fill=factor(variable,levels=c('education','deaths','gdp'),labels=c('Education','YLLs','GDP'))),stat='identity') + facet_grid(scencountry~factor(igroup,levels=c('LLMIC','UMIC','HIC'))) + 
...
voiall <- readRDS('code/results/voi_BAU_Origin.Rds')
...
```{r dvoiBAU,fig.cap='Decision VOI for BAU. Which parameters would enable the decision maker to choose a strategy that minimises expected losses for BAU vaccination?',eval=T,echo=F,warning=F,message=F,fig.height=12}

voiall <- readRDS('code/results/decisionvoiBAU_Origin.Rds')*100
rownames(voiall) <- gsub('Deaths:','YLLs:',rownames(voiall))
rownames(voiall) <- gsub('School:','Education:',rownames(voiall))
...
```{r dvoi100,fig.cap='Decision VOI for 100 days. Which parameters would enable the decision maker to choose a strategy that minimises expected losses for 100-day vaccination?',eval=T,echo=F,warning=F,message=F,fig.height=12}

voiall <- readRDS('code/results/decisionvoi100 days_Origin.Rds')*100
rownames(voiall) <- gsub('Deaths:','YLLs:',rownames(voiall))
rownames(voiall) <- gsub('School:','Education:',rownames(voiall))
...
ggplot(subset(allresults,igroup=='UMIC'&strategy=='School Closures'),aes(x=qbeta(Internet,3.77,2.91)*100,y=School)) +
...
ggplot(subset(allresults,igroup=='UMIC'&strategy=='No Closures'),aes(y=GDP_loss,x=Food_sector*100)) +
  geom_point(colour='navyblue') +
  theme_bw(base_size=15) +
  theme_bw(base_size=15) + geom_smooth(formula=y~ns(x,df=3),method='glm',colour='hotpink',se = F,size=2) +
...
ggplot(subset(allresults,igroup=='UMIC'&strategy=='No Closures')) +
...
The distribution of people among sectors of the workforce impacts the numbers of contacts made. In order to propagate these influences through the model, we sample a null distribution of $\beta$ values from the generating model using the average number of candidate infectees (CI, where CI $=R_0/\beta$). We regress $\beta$ against $R_0$ to get a one-to-one mapping from the $R_0$ we sample to the $\beta$ used in simulations. This means that a single disease sample has a fixed $\beta$, and its $R_0$ will depend on the country characteristics.
...
```{r bmi, fig.cap="Distributions of mean BMI among adults by country income level.",echo=F,warning=F,message=F}
...
mic <- fitdistr(subset(bmi,!is.na(FactValueNumeric)&IncomeGroup%in%c('Upper middle income'))$FactValueNumeric,"normal")
llmic <- fitdistr(subset(bmi,!is.na(FactValueNumeric)&IncomeGroup%in%c('Lower middle income','Low income'))$FactValueNumeric,"normal")


```

We model these values with normal distributions. For LLMICs, we have mean and standard deviation parameters `r round(llmic$estimate['mean'],2)` and  `r round(llmic$estimate['sd'],2)`. For UMICs, we have parameters `r round(mic$estimate['mean'],2)` and  `r round(mic$estimate['sd'],2)`. For HICs, we have parameters `r round(hic$estimate['mean'],2)` and  `r round(hic$estimate['sd'],2)`.
