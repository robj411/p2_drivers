---
title: "Projecting the public health, economic and educational benefits of CEPI’s 100 day mission with the DAEDALUS model"
...
scenario_levels <- 1:5
scenario_names <- c('BAU',paste0('Scenario ',scenario_levels[-1]-1))
...
We will model four mitigation strategies that decision maker may adopt once the novel pathogen has been identified. These are "unmitigated," "adaptive economic closures," "school closures," and "elimination." The policy defines government-mandated actions. In addition, we model behaviour changes in response to case numbers that impact on transmission.
...
We will designate the policy that minimises costs across health, GDP and education to be the mitigation choice for each country type and income level and vaccination scenario. 
...
Numbers of deaths are translated into discounted years of life lost (YLL). This takes into account the life expectancy of each person who dies. YLL are valued using the value of a statistical life, which we estimate as being 160 times GDP per capita [@Robinson2019].
...
| **Mitigation policy** | No closures | Unmitigated pandemic |
...
<!-- These distributions show that vulnerability to a pandemic depends on a country's income level together with its mitigation policy. For example, the elimination policy appears to have the lowest distribution for overall cost for HICs among all strategies, whereas it appears no lower for UMICs and perhaps higher for LLMICs. This will be because of the factors that vary systematically across countries of different income levels. Likewise, the heavier tails in the distributions for YLL for HICs is likely due to the age distributions in HICs being heavier in older ages. These cross-country differences are one of the focuses of the development of the model. -->
...
for (i in 1:length(income_levels)){
    ...
    for (k in 1:length(strategies)){
        inp3 <- strategies[k];
        results <- read.csv(paste0('code/results/outputs_',inp3,'_',income_level,'_scen1.csv'),header=T);
        results$Policy <- inp3
...
vioplot <- melt(allresults[,(nvar-3):ncol(allresults)],id.vars=c('Policy','igroup'))
vioplot$igroup <- factor(vioplot$igroup,levels=c('LLMIC','UMIC','HIC'))
vioplot$Policy <- factor(vioplot$Policy,levels=c('No Closures','School Closures','Economic Closures','Elimination'))
vioplot$variable <- factor(vioplot$variable,
                           levels=c('Cost','YLL','School','GDP_loss'),
                           labels=c('Cost','YLL','Education','GDP'))
  ...
  p[[sl]] <- list()
    ...
    for (i in 1:length(income_levels)){
        ...
        for (k in 1:length(strategies)){
            ...
            results <- read.csv(paste0('code/results/outputs_',inp3,'_',income_level,'_scen',sl,'.csv'),header=T);
            results$Deaths <- NULL
            results$Policy <- inp3
            results$igroup <- income_level
            results$samplei <- 1:nrow(results)
            # colnames(results)[colnames(results)=='dYLLs'] <- 'YLL'
    ...
    vioplot <- melt(allresults[,c(outcomenames,'Policy','igroup')],id.vars=c('Policy','igroup'))
    vioplot$igroup <- factor(vioplot$igroup,levels=c('LLMIC','UMIC','HIC'))
    vioplot$Policy <- factor(vioplot$Policy,levels=c('No Closures','School Closures','Economic Closures','Elimination'))
    vioplot$variable <- factor(vioplot$variable,
                               levels=c('Cost','YLL','School','GDP_loss'),
    ...
    p[[sl]] <- ggplot(vioplot,aes(fill=Policy,colour=Policy, y=value, x=Policy)) + 
      ...
      facet_grid(variable~igroup, scales="free") + 
        ...
        scale_colour_viridis(discrete=T, name="",option='inferno',direction=-1) +
    ...
    meantab <- allresults[,.(deaths=mean(YLL),
    ...
    tabres <- setDT(allresults)[,list(costs=paste0(round(quantile(Cost,c(0.25,0.75)),1),collapse='--'),
                            deaths=paste0(round(quantile(YLL,c(0.25,0.75)),1),collapse='--'),
                            education=paste0(round(quantile(School,c(0.25,0.75)),1),collapse='--'),
                            gdp=paste0(round(quantile(GDP_loss,c(0.25,0.75)),1),collapse='--')
                            ),by=.(Policy,igroup)]
    colnames(tabres) <- c('Policy','Income group','Total costs','YLL','Education','GDP')
...
ggplot(meltmeantab) + 
      geom_bar(aes(x=factor(Policy,levels=c('No Closures','School Closures','Economic Closures','Elimination')),
                   y=value,fill=factor(variable,levels=c('education','deaths','gdp'),labels=c('Education','YLL','GDP')),
...
for(sl in scenario_levels){
    if(sl>1){
      scenario <- scenario_names[sl]
      difftab <- as.data.frame(readRDS(paste0('code/results/difftab_',sl,'.Rds')))
      # colnames(difftab)[colnames(difftab)=='dYLLs'] <- 'YLL'
      nvar <- which(colnames(difftab)=='GDP_loss')
      for(i in (nvar-3):nvar){
        difftab[,i] = difftab[,i]/difftab$gdp*100;
      }
      difftab$scen <- scenario
      # colnames(difftab)
      # for (i in colnames(difflist)) if(!i%in%colnames(difftab)) difflist[[i]] <- NULL
...
vioplot <- melt(difflist[,colnames(difflist)%in%c('Cost','YLL','School','GDP_loss','igroup','scen')],id.vars=c('igroup','scen'))
vioplot$igroup <- factor(vioplot$igroup,levels=c('LLMIC','UMIC','HIC'))
vioplot$variable <- factor(vioplot$variable,
                           levels=c('Cost','YLL','School','GDP_loss'),
                           labels=c('Total','YLL','Education','GDP'))

ggplot(vioplot,aes(fill=scen, colour=scen, y=value, x=scen)) + 
    geom_violin(position="dodge",scale='width') +
  facet_grid(variable~igroup, scales="free") + 
    scale_fill_viridis(discrete=T, name="") +
...
meantab <- setDT(difflist)[,.(deaths=mean(YLL),
                        education=mean(School),
                        gdp=mean(GDP_loss)),by=.(igroup,scen)]

tabres <- setDT(difflist)[,list(costs=paste0(signif(quantile(Cost,c(0.25,0.75)),2),collapse='--'),
                        deaths=paste0(signif(quantile(YLL,c(0.25,0.75)),2),collapse='--'),
                        education=paste0(signif(quantile(School,c(0.25,0.75)),2),collapse='--'),
                        gdp=paste0(signif(quantile(GDP_loss,c(0.25,0.75)),2),collapse='--')
                        ),by=.(scen,igroup)]

tabres[,igroup:=factor(igroup,levels=c('LLMIC','UMIC','HIC'))]
tabres[,Scenario:=scen]
setorder(tabres,scen,igroup)
colnames(tabres) <- c('Scenario','Income group','Total value','YLL','Education','GDP','s')


write.csv(tabres,'figures/resultsdiff.csv',row.names = F)

tabres <- tabres[,.(Scenario,`Income group`,`Total value`,YLL,Education,GDP)]

lg <- 'Value of vaccines as a percent of GDP: differences between BAU and scenarios. Mean values and 75% prediction intervals. \\label{tab:tabresdiff}'
if (!knitr::is_html_output(excludes = "markdown")) { 
...
tabresGDP <- difflist[,list(costs=paste0(format_to_print2(quantile(Cost*gdp/100,c(0.25,0.75))),collapse='--'),
                        deaths=paste0(format_to_print2(quantile(YLL*gdp/100,c(0.25,0.75))),collapse='--'),
                        education=paste0(format_to_print2(quantile(School*gdp/100,c(0.25,0.75))),collapse='--'),
                        gdp=paste0(format_to_print2(quantile(GDP_loss*gdp/100,c(0.25,0.75))),collapse='--')
                        ),by=.(scen,igroup)]

tabresGDP[,igroup:=factor(igroup,levels=c('LLMIC','UMIC','HIC'))]
tabresGDP[,Scenario:=scen]
setorder(tabresGDP,scen,igroup)
colnames(tabresGDP) <- c('Scenario','Income group','Total value','YLL','Education','GDP','s')

tabresGDP <- tabresGDP[,.(Scenario,`Income group`,`Total value`,YLL,Education,GDP)]

lg <- 'Value of vaccines in $: differences between BAU and scenarios. Mean values and 75% prediction intervals. \\label{tab:tabresdiffGDP}'
if (!knitr::is_html_output(excludes = "markdown")) { 
  pander(tabresGDP,caption=lg)
}else{
    x <- knitr::kable(tabresGDP,escape=F, "html",caption=lg)
  kableExtra::kable_styling(x,full_width = F,latex_options = "HOLD_position")
}
...
ggplot(choicestab) +  
  geom_bar(aes(x=scenario,
               y=N/20.48,
               fill=factor(policy,levels=c('No Closures','School Closures','Economic Closures','Elimination'))),stat='identity') + 
...
```{r importtime,fig.cap='Hospital occupancy at time of response.',echo=F,warning=F,message=F,fig.height=5,fig.width=6,eval=F}

ggplot(subset(allallresults,Policy=='No Closures'&scenario=='BAU'),aes(x=Hospital_occupancy_at_response,y=..density..)) +
...
```{r iseqec,fig.cap='Closures for HICs under the Economic Closures policy.',echo=F,warning=F,message=F,fig.height=5,fig.width=7}
## iseq ########################
...
```{r iseqelim,fig.cap='Closures for HICs under the Elimination policy. *Future development: find the most open configuration for "Closed" that will allow Rt to remain below 1.*',echo=F,warning=F,message=F,fig.height=5,fig.width=7}
## iseq ########################
...
ggplot(subset(allallresults,Policy=='No Closures'&scenario=='BAU'),aes(x=t_import,y=Tres)) +
  # geom_histogram(colour='white',fill='white') +
  geom_point(aes(shape=t_import>Tres),size=3,show.legend=F,alpha=.5,colour='navyblue') +
...
```{r vsy,fig.cap='The relationship between policy choice and value of a school year.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}

setDT(allallresults)
allallresults[,mincost:=min(Cost),by=.(igroup,scenario,samplei)]
allallresults[,keeprow:=Cost==mincost]
allallresults$Policy <- factor(allallresults$Policy,levels=c('No Closures','School Closures','Economic Closures','Elimination'))

# ggplot(subset(allallresults,keeprow==T)) + 
#   geom_density(aes(x=VSY/gdp,colour=Policy,group=Policy),position='identity',alpha=.5,linewidth=1.25) +
...
keptresults[,qVSY:=rank(vsy)/.N,by=.(igroup,scenario)]

ggplot(keptresults) + 
  geom_histogram(aes(x=qVSY,y=stat(count / sum(count)),fill=Policy,group=Policy),position='fill',bins = 10) +
...
```{r vsyeffectiveness,fig.cap='The relationship between policy choice and the effectiveness of remote teaching.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}

# ggplot(subset(allallresults,keeprow==T)) + 
#   geom_density(aes(x=Remote_teaching_effectiveness,colour=Policy,group=Policy),position='identity',alpha=.5,linewidth=1.25) +
#   facet_grid(factor(igroup,levels=c('LLMIC','UMIC','HIC'))~vax) + 
#   theme_bw(base_size = 13) +
#   theme(axis.text.x = element_text(angle = 45,  hjust=1,vjust=1),
...
ggplot(subset(allallresults,keeprow==T)) + 
  geom_histogram(aes(x=remote_teaching_effectiveness,y=stat(count / sum(count)),fill=Policy,group=Policy),position='fill',bins = 10) +
  facet_grid(factor(igroup,levels=c('LLMIC','UMIC','HIC'))~scenario) + 
  theme_bw(base_size = 11) +
  scale_x_continuous(expand=c(0,0)) +
  scale_y_continuous(expand=c(0,0)) +
...
```{r importtoresponse,fig.cap='The relationship between policy choice and the duration between import time and response time.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}


# ggplot(subset(allallresults,keeprow==T)) + 
#   geom_vline(aes(xintercept=0),colour='grey',linewidth=1.2) +
#   geom_density(aes(x=Response_time-Importation_time,colour=Policy,group=Policy),position='identity',alpha=.5,linewidth=1.25) +
#   facet_grid(factor(igroup,levels=c('LLMIC','UMIC','HIC'))~vax) + 
#   theme_bw(base_size = 13) +
#   theme(axis.text.x = element_text(angle = 45,  hjust=1,vjust=1),
#         legend.position = 'top') + labs(colour='',x='Response time minus importation time',y='Density')

keptresults[,qtime:=rank(Tres-t_import)/.N,by=.(igroup,scenario)]
keptresults[,qtime0:=qtime[which.min(abs(Tres-t_import))],by=.(igroup,scenario)]

ggplot(keptresults) + 
  geom_histogram(aes(x=qtime,y=stat(count / sum(count)),fill=Policy,group=Policy),position='fill',bins = 10) +
...
```{r vly,fig.cap='The relationship between policy choice and value of a life year.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}

keptresults[,qVLY:=rank(vly)/.N,by=.(igroup,scenario)]

ggplot(keptresults) + 
  geom_histogram(aes(x=qVLY,y=stat(count / sum(count)),fill=Policy,group=Policy),position='fill',bins = 10) +
  facet_grid(factor(igroup,levels=c('LLMIC','UMIC','HIC'))~scenario) + 
  theme_bw(base_size = 11) +
  scale_x_continuous(expand=c(0,0)) +
  scale_y_continuous(expand=c(0,0)) +
  scale_fill_viridis(discrete=T, name="",option='inferno',direction=-1) +
  theme(axis.text.x = element_text(angle = 45,  hjust=1,vjust=1),
        legend.position = 'top') + labs(fill='',x='Value of a life year (quantile)',y='Proportion')

```


```{r seed,fig.cap='The relationship between policy choice and the size of the seed.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}

keptresults[,qseedsize:=-seedsize,by=.(igroup,scenario)]

ggplot(keptresults) + 
  geom_histogram(aes(x=qseedsize,y=stat(count / sum(count)),fill=Policy,group=Policy),position='fill',bins = 10) +
  facet_grid(factor(igroup,levels=c('LLMIC','UMIC','HIC'))~scenario) + 
  theme_bw(base_size = 11) +
  scale_x_continuous(expand=c(0,0)) +
  scale_y_continuous(expand=c(0,0)) +
  scale_fill_viridis(discrete=T, name="",option='inferno',direction=-1) +
  theme(axis.text.x = element_text(angle = 45,  hjust=1,vjust=1),
        legend.position = 'top') + labs(fill='',x='Size of the seed (log base 10)',y='Proportion')
...
Some distributions in Figure \@ref(fig:prealpha) are bimodal. For example, GDP loss under the Economic Closures policy have peaks in density close to zero and also far from zero, with a low-density region in between. Here, in some samples (low GDP loss), the epidemic was not severe enough to warrant closures. This could be because R$_0$ was low, because hospital capacity was high, or both. Similarly, under the Elimination policy, there is an area of high density at higher costs where the policy does not work: due to R$_0$ being high, and/or the capacity to test being low, case numbers are not brought low enough and severe economic closures persist throughout, leading to high GDP loss.
...
```{r voidiff,fig.cap='\\footnotesize Value of information for 100-day vaccine value.',echo=F,warning=F,message=F,fig.height=10}

param_cutoff <- c('Remote quantile','Time to death')

voiall <- readRDS('code/results/voidiff.Rds')
...
ggplot(df, aes(x, y, fill = factor(xs,levels=1:(length(voibrks)-1),labels=voilbs))) +
  ...
  theme(
    ...
    legend.position="right", legend.spacing.x = unit(0.15, 'cm'),legend.box.spacing = unit(0.1, 'cm')) +               
  guides(fill = guide_legend(#nrow = 1,
    title.theme = element_text(size=10),title.vjust = .8,title.hjust = 1,#title.position = "left",      
    #label.position="bottom", 
    label.hjust = 0, #label.vjust = 1.3, 
...
```{r voi,fig.cap='\\footnotesize Value of information for BAU.',echo=F,warning=F,message=F,fig.height=10}

voiall <- readRDS('code/results/voi_365_0.Rds')
rownames(voiall) <- gsub('dYLLs:','YLL:',rownames(voiall))
rownames(voiall) <- gsub('Deaths:','YLL:',rownames(voiall))
...
```{r dvoiBAU,fig.cap='Decision VOI for BAU. Which parameters would enable the decision maker to choose a policy that minimises expected losses for BAU vaccination?',eval=T,echo=F,warning=F,message=F,fig.height=12,include=F,eval=F}
voiall <- c()
for(i in scenario_levels){
    voitab <- readRDS(paste0('code/results/decisionvoi',i,'.Rds'))*100
...
```{r dvoi100,fig.cap='Decision VOI for 100 days. Which parameters would enable the decision maker to choose a policy that minimises expected losses for 100-day vaccination?',eval=T,echo=F,warning=F,message=F,fig.height=12,include=F,eval=F}
...
To understand the relationship between a parameter and an outcome, e.g. whether the relationship is linear and increasing or decreasing, whether it is nonlinear, whether it is driven by extreme values, they must be plotted against each other. Likewise, a relationship between a parameter pair and an outcome can be plotted, e.g. Figure \@ref(fig:tourismgdp): for middle-income countries in a pre-Alpha SARS-CoV-2 pandemic with the "No Closures" policy, GDP loss is greatest when both the food and accommodation services GVA as fraction of GDP and international tourism as a fraction of tourism are high.


```{r R0cost,fig.cap='Relationship between R$_0$ and total costs for UMICs with the "School Closures" policy.',echo=F,warning=F,message=F}

ggsave(ggplot(subset(allallresults,igroup=='UMIC'&Policy=='School Closures'&scenario=='BAU'),aes(x=R0,y=Cost)) +
  geom_point(colour='navyblue') +
  theme_bw(base_size=15) + geom_smooth(formula=y~ns(x,df=3),method='glm',colour='hotpink',se = F,size=2) +
  labs(x=bquote(R[0]),y='Total socio-economic cost, % of GDP'),filename='figures/R0costBAU.png')
ggsave(ggplot(subset(allallresults,igroup=='UMIC'&Policy=='School Closures'&scenario=='BAU'),aes(x=R0,y=log(Cost))) +
  geom_point(colour='navyblue') +
  theme_bw(base_size=15) + geom_smooth(formula=y~ns(x,df=3),method='glm',colour='hotpink',se = F,size=2) +
  labs(x=bquote(R[0]),y='Log total socio-economic cost, % of GDP'),filename='figures/R0logcostBAU.png')

setBAU <- subset(allallresults,igroup=='UMIC'&Policy=='School Closures'&scenario=='BAU')
setscen4 <- subset(allallresults,igroup=='UMIC'&Policy=='School Closures'&scenario=='Scenario 4')
setBAU$averted <- setBAU$Cost - setscen4$Cost
ggsave(ggplot(setBAU,aes(x=R0,y=averted)) +
  geom_point(colour='navyblue') +
  theme_bw(base_size=15) + 
  labs(x=bquote(R[0]),y='Total socio-economic saving, % of GDP'),filename='figures/R0saving.png',width=10,height=6)

```

```{r tourismsecgdp,fig.cap='Relationship between the "tourism sector" size and GDP loss for Origin LLMICs with the "No Closures" policy.',echo=F,warning=F,message=F,eval=F}
ggplot(subset(allallresults,igroup=='LLMIC'&Policy=='No Closures'&BPSV=='No'&vaccination_level==365),
...
# with(subset(allallresults,igroup=='LLMIC'&Policy=='No Closures'&BPSV=='No'&vaccination_level==365),plot(Food_sector,rank(Food_sector)))

```

```{r tourismgdp,fig.cap='Relationship between tourism parameters and GDP loss for LLMICs with the "No Closures" policy.',echo=F,warning=F,message=F,eval=F}
...
We found that the tourism-related variables impacted GDP loss for the No Closure policy, which is because GDP losses will be suffered only in this sector when there is no mandated closure. This means the GDP loss will depend on the extent to which the economy is depending on this sector. We found that Education losses depend to some extent on internet coverage, because our model assumes that internet access can be used to mitigate losses by enabling remote learning. We found little effect of the size of the agricultural sector.
