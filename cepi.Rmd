...
for(vl in 1:length(vaccination_levels)){
  ...
  for(ct in 1:length(countrytype_levels)){
    ...
    meltmean$igroupcountry <- paste0(meltmean$igroup,', ',countrytype)
    meltmean$scen <- vaccination_level
...
```{r expectedvaluesbar,fig.cap='Expected values of model results.',echo=F,warning=F,message=F,fig.height=7.5,fig.width=6}

meltmeantab$igroupcountry <- factor((meltmeantab$igroupcountry), 
                                    levels=c('LLMIC, Origin','LLMIC, Secondary',
                                             'UMIC, Origin','UMIC, Secondary',
                                             'HIC, Origin','HIC, Secondary'))
setDT(meltmeantab)
meltmeantab[,cost:=sum(value),by=.(scen,igroupcountry,strategy)]
meltmeantab[,mincost:=cost==min(cost),by=.(scen,igroupcountry)]
ggplot(meltmeantab) + 
  ...
  facet_grid(igroupcountry~factor(scen,levels=c('BAU','100 days','BPSV'))) + 
...
ggplot(vioplot,aes(fill=igroup, y=value, x=igroup)) + 
  ...
  ggplot(subset(vioplot,grepl('Secondary',CountryVaccine)),aes(fill=igroup, y=value, x=igroup)) + 
    geom_violin(position="dodge", alpha=0.5, outlier.colour="transparent") +
  facet_grid(factor(CountryVaccine,labels=c('100-day vaccine','BPSV'))~variable, scales="free") + 
    scale_fill_viridis(discrete=T, name="") +
    theme_bw(base_size = 15)  +
    xlab("") +
    ylab("Percent of GDP") +
  ...
  ggsave(p,filename='diffviolin.png',width=9,height=4.5)
...
setorder(tabres,Vaccine,igroup)
...
```{r voi100,fig.cap='\\footnotesize Value of information for 100-day vaccine value, Origin country.',echo=F,warning=F,message=F,fig.height=10}

param_cutoff <- 'Start vaccination'

voiall <- readRDS('code/results/voidiff_Origin_100 days.Rds')
...
```{r dvoi100,fig.cap='Decision VOI for 100 days. Which parameters would enable the decision maker to choose a strategy that minimises expected losses for 100-day vaccination?',eval=T,echo=F,warning=F,message=F,fig.height=12,include=F}
...
ggplot(subset(allallresults,igroup=='UMIC'&strategy=='School Closures'&countrytype=='Origin'&vaccination_level=='BAU'),aes(x=R0,y=Cost)) +
...
ggplot(subset(allallresults,igroup=='LLMIC'&strategy=='No Closures'&countrytype=='Origin'&vaccination_level=='BAU'),
       aes(y=GDP_loss,x=Food_sector*100)) +
  geom_point(colour='navyblue') +
  theme_bw(base_size=15) +
  theme_bw(base_size=15) + geom_smooth(formula=y~ns(x,df=3),method='glm',colour='hotpink',se = F,size=2) +
