...
params:
  lbfile: './data/20250922 updated scenario delivery and costing.xlsx'
---
...
tab <- readxl::read_xlsx(params$lbfile, sheet = 'Delivery')
...
ggplot(subset(plotscens,!Scenario%in%c('BAU2'))) + 
  geom_line(aes(x=Day,y=Percent,colour=Scenario),linewidth=.6,alpha=.75) +
...
costresults <- readxl::read_xlsx(params$lbfile,sheet = 3)
...
```{r costresultsbau2,echo=F,warning=F,message=F}
costresults <- readxl::read_xlsx(params$lbfile,sheet = 4)
costresults$`Cost Category`[which(costresults$`Cost Category`=='Difference to BAU')+0:4] <- 'Difference to BAU'
costresults <- costresults[!is.na(costresults$`Cost Category`),]
costresults <- costresults[costresults$`Cost Category`!='Cost Category',]
newClabels <- c('Mean','C5','C25','C50','C75','C95')
...
subexp <- expvals1#[,!colnames(expvals)%in%scenario_names[-c(scen_to_keep)],with=F]
