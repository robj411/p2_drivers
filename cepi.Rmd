...
for(vl in 1:length(vaccination_levels)){
  ...
  for(ct in 1:length(countrytype_levels)){
    ...
    tabres$`Vaccination level` <- vaccination_level
    tabres$`Country type` <- countrytype
...
```{r choicestab,fig.cap='Frequency of mitigation strategy choices.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}

choicestab <- readRDS(paste0('code/results/choicestab.Rds'))

ggplot(choicestab) +  
  geom_bar(aes(x=countrytype,
               y=N/20.48,
               fill=factor(strategy,levels=c('No Closures','School Closures','Economic Closures','Elimination'))),stat='identity') + 
  facet_grid(factor(igroup,levels=c('LLMIC','UMIC','HIC'))~factor(vaccination_level,levels=c('BAU','100 days','BPSV'))) + 
  theme_bw(base_size=15) +
