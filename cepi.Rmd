...
```{r tabresdiff,tab.cap='Expected difference between BAU and 100 days. Mean values and 95% prediction intervals.',echo=F,warning=F,message=F}
...
lg <- 'Expected difference between BAU and 100 days, and between 100 days and BPSV. Mean values and 95% prediction intervals. \\label{tab:tabresdiff}'
...
for (i in 1:length(income_levels)){
    for (k in 1:length(strategies)){
        ...
        results <- read.csv(paste0('code/results/VOI_',inp3,'_',income_level,'_BAU_Spillover.csv'),header=T);
...
```{r BAUviolinspill,fig.cap='Model results for the BAU scenario in spillover countries.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}
p[[1]][[1]]
```
...
```{r violin100spill,fig.cap='Model results for the 100-day scenario in spillover countries.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}
p[[2]][[1]]
```
...
```{r importtime,fig.cap='Model results for the 100-day scenario in secondary countries.',echo=F,warning=F,message=F,fig.height=5,fig.width=6}
ggplot(subset(allallresults,countrytype=='Secondary'&vaccination_level=='100 days'),aes(x=R0,y=log(Cost),colour=Importation_time>Response_time)) +
  geom_point(alpha=.5) +
  facet_grid(strategy~igroup)
```
...
```{r barBAUSpillover,fig.cap='Expected values of model results.',echo=F,warning=F,message=F,fig.height=7.5,fig.width=6}
...
voiall <- readRDS('code/results/voi_BAU_Spillover.Rds')
...
voiall <- readRDS('code/results/decisionvoiBAU_Spillover.Rds')*100
rownames(voiall) <- gsub('Deaths:','YLLs:',rownames(voiall))
rownames(voiall) <- gsub('School:','Education:',rownames(voiall))
...
voiall <- readRDS('code/results/decisionvoi100 days_Spillover.Rds')*100
rownames(voiall) <- gsub('Deaths:','YLLs:',rownames(voiall))
rownames(voiall) <- gsub('School:','Education:',rownames(voiall))
