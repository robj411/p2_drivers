...
for(vl in 1:length(vaccination_levels)){
  ...
  for(bl in 1:length(bpsv_levels)){
    ...
    p[[vl]][[bl]] <- ggplot(vioplot,aes(fill=Policy, y=value, x=Policy)) + 
...
```{r interneted,fig.cap='Relationship between R$_0$ and total costs for UMICs with the "School Closures" policy.',echo=F,warning=F,message=F}

ggplot(subset(allallresults,igroup=='UMIC'&Policy=='School Closures'&BPSV=='No'&vaccination_level==365),aes(x=R0,y=Cost)) +
