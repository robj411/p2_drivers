...
for (i in 1:length(income_levels)){
    ...
    for (k in 1:length(strategies)){
        inp3 <- strategies[k];
        results <- read.csv(paste0('code/results/outputs_',inp3,'_',income_level,'_365_0.csv'),header=T);
        results$Policy <- inp3
  ...
  for(bl in 1:length(bpsv_levels)){
    ...
    for (i in 1:length(income_levels)){
        ...
        for (k in 1:length(strategies)){
            ...
            results <- read.csv(paste0('code/results/outputs_',inp3,'_',income_level,'_',vaccination_level,'_',bpsv,'.csv'),header=T);
            results$Deaths <- NULL
            results$Policy <- inp3
            results$igroup <- income_level
    ...
    p[[vl]][[bl]] <- ggplot(vioplot,aes(fill=Policy,colour=Policy, y=value, x=Policy)) + 
...
tabresall <- cbind(tabresall[,9],tabresall[,-c(7:9)])
...
tabres[,Scenario:=paste0(Vaccine,' days, ',c('no BPSV','BPSV')[BPSV]),by=.(Vaccine,BPSV)]
tabres[,Vaccine:=factor(Vaccine,levels=c(365,100))]
setorder(tabres,BPSV,Vaccine,igroup)
colnames(tabres) <- c('BPSV','Vaccine','Income group','Total value','YLL','Education','GDP','Scenario')
...
tabresGDP[,Scenario:=paste0(Vaccine,' days, ',c('no BPSV','BPSV')[BPSV]),by=.(Vaccine,BPSV)]
tabresGDP[,Vaccine:=factor(Vaccine,levels=c(365,100))]
setorder(tabresGDP,BPSV,Vaccine,igroup)
colnames(tabresGDP) <- c('BPSV','Vaccine','Income group','Total value','YLL','Education','GDP','Scenario')
...
ggplot(subset(allallresults,Policy=='No Closures'&vaccination_level==365&BPSV=='No'),aes(x=Hospital_occupancy_at_response,y=..density..)) +
...
ggplot(subset(allallresults,Policy=='No Closures'&vaccination_level==365&BPSV=='No'),aes(x=t_import,y=Tres)) +
  # geom_histogram(colour='white',fill='white') +
...
keptresults[,qVSY:=rank(vsy)/.N,by=.(igroup,vax)]
...
keptresults[,qtime:=rank(Tres-t_import)/.N,by=.(igroup,vax)]
keptresults[,qtime0:=qtime[which.min(abs(Tres-t_import))],by=.(igroup,vax)]
...
keptresults[,qVLY:=rank(vly)/.N,by=.(igroup,vax)]
...
keptresults[,qseedsize:=-seedsize,by=.(igroup,vax)]
...
ggplot(df, aes(x, y, fill = factor(xs,levels=1:(length(voibrks)-1),labels=voilbs))) +
  ...
  scale_y_continuous(name='',breaks=rev(yvals),labels=strats,expand=c(0,0),
                     sec.axis=sec_axis(trans='identity',breaks=seq(2,9,by=3.1),labels=rev(unique(splitnames[,1])))) +
  coord_flip()
```
...
ggsave(ggplot(subset(allallresults,igroup=='UMIC'&Policy=='School Closures'&BPSV=='No'&vaccination_level==365),aes(x=R0,y=Cost)) +
  geom_point(colour='navyblue') +
  theme_bw(base_size=15) + geom_smooth(formula=y~ns(x,df=3),method='glm',colour='hotpink',se = F,size=2) +
  labs(x=bquote(R[0]),y='Total socio-economic cost, % of GDP'),filename='R0cost365.png')

set100 <- subset(allallresults,igroup=='UMIC'&Policy=='School Closures'&BPSV=='No'&vaccination_level==100)
set365 <- subset(allallresults,igroup=='UMIC'&Policy=='School Closures'&BPSV=='No'&vaccination_level==365)
set365$averted <- set365$Cost - set100$Cost
ggsave(ggplot(set365,aes(x=R0,y=averted)) +
  geom_point(colour='navyblue') +
  theme_bw(base_size=15) + 
  labs(x=bquote(R[0]),y='Total socio-economic saving, % of GDP'),filename='R0saving.png',width=10,height=6)
