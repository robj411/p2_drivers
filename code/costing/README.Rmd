...
```{r}
eqtext = '\\hat{P}_i = \\left\\{\\begin{array}{lr}1 & i=0 \\\\ 
...
for $i \in \{0,1,2,3,4\}$, with $i=4$ corresponding to licensure. The cost of each phase is $T_i$, a weighted average of experienced and inexperienced manufacturers (with $\omega = `r rawpar$inex_weight`$):

$$T_{i} = \omega T_i^{(n)} + (1-\omega)T_i^{(e)}.$$ 

Then the total weighted cost for phases 0 through 2 for $N^{\text{(BPSV)}}$ candidates is
...
```{r}
eqtext = 'D_s^{\\text{(BP-resRD)}} = \\left\\{\\begin{array}{lr}N^{\\text{(BPSV)}}\\hat{P}_3\\left(\\frac{W_3^{(B)}}{52Y_3^{(B)}}T_3^{(e)} + (1+I) P_3 T_4\\right) \\; & \\; s=1 \\\\
0  \\; & \\; s\\neq 1
\\end{array}\\right.
(\\#eq:bpsvresrd)'
if (knitr::is_html_output()) {
...
Facility transition occurs $I_0=7$ weeks before vaccine approval, which in turn depends on R\&D investments. We have three levels in our scenarios, corresponding to a 100 Days Mission, 200 days, and 365 days. The total weeks taken for vaccine approval can be written as follows:
