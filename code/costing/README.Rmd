...
The cost of goods sold is $G = `r rawpar$cost_cogs`$ USD per dose. Then the cost of drug substance, accounting for the fill/finish margin $M_f = `r round(rawpar$cost_ff,2)`$ and the profit margin $M_p = `r round(rawpar$profit,2)`$, is $G(1-M_f)(1+M_p) = `r round(rawpar$cost_cogs*(1-rawpar$cost_ff)*(1+rawpar$profit),2)`$ USD per dose. The reserve is replenished every $Y_{rep} = `r rawpar$bpsv_replenishment`$ years based on the storage characteristics for Ervebo [@WorldHealthOrganization2019a]. Then the annual cost to maintain the reserve of $A_4 =`r format_to_print(rawpar$bpsv_inv_res)`$ doses is
...
```{r}
eqtext = 'T_{\\zeta,i}^{(e)} = (1+\\pi)\\frac{W_{i;\\zeta}^{(S)}}{52Y_{i}^{(B)}}T_i^{(e)}'
printmath(eqtext)
```


where $\pi = `r rawpar$inflation`$ is inflation from 2018 to 2025, $T_i^{(e)}$ is the cost per Phase $i$ of experienced developers (i.e. we assume only experienced manufacturers undertake reactive R\&D), $Y_{i}^{(B)}$ is the usual phase duration in normal times in years, and $W_{i;\zeta}^{(S)}$ is its expected duration in weeks given enabling investments made prior to the outbreak. 
...
One BPSV candidate that has passed through phases 0 to 2 prior to the outbreak goes through Phase 3 during the response. The duration is $W_{3; 365}^{(S)}=`r rawpar$weeks_P3_365`$ weeks. Thus we write the BPSV R\&D response cost


```{r}
eqtext = 'D_s^{\\text{(BP-resRD)}} = \\begin{cases}\\left( (1+\\pi)\\frac{W_{3; 365}^{(S)}}{52Y_3^{(B)}}T_3^{(e)} + T_4\\right) \\; & \\; s=1 \\\\
0  \\; & \\; s\\neq 1
\\end{cases}
(\\#eq:bpsvresrd)'
printeq(eqtext)
...
The cost per dose comes from the cost of goods sold (COGS), $G = `r rawpar$cost_cogs`$ USD, adjusted for a profit margin ($M_p = `r rawpar$profit`$) and transportation margin ($M_t = `r rawpar$cost_travel`$).
...
```{r}
eqtext = 'W_{\\zeta}^{(S)} = \\sum_{i=0}^3 W_{i;\\zeta}^{(S)}'
printmath(eqtext)
```
...
```{r}
eqtext = 'w_s^{(0)} = W_{\\zeta(s)}^{(S)} - I_0 = \\begin{cases}45 & s\\in\\lbrace 0, 1, 2, 3, 10\\rbrace \\\\ 
...
where $I_R = `r rawpar$weeks_init_res`$ is the number of weeks to initial manufacturing for reserved capacity, and $C_R = `r rawpar$weeks_scale_res`$ is its number of weeks to scale up to full capacity; $I_B = `r rawpar$weeks_init_bui`$ is the number of weeks to initial manufacturing for newly built capacity, and $C_B = `r rawpar$weeks_scale_bui`$ is its number of weeks to scale up to full capacity; and
...
where $I_{E,0} = `r rawpar$weeks_init_ex_nb`$ and $I_{E,1} = `r rawpar$weeks_init_ex_bp`$ are the number of weeks to initial manufacturing for existing and unreserved capacity (i.e. manufacturing conversion for SSVs is faster in the presence of BPSVs as we assume that they are manufactured using the same platform), and $C_E = `r rawpar$weeks_scale_ex`$ is its number of weeks to scale up to full capacity.
...
The duration of the Phase 3 trial is $W_{3; 365}^{(S)} = `r rawpar$weeks_P3_365`$ weeks. The time to manufacturing transition is $I_R = `r rawpar$weeks_init_res`$ weeks, and the time to manufacturing scale-up $C_R = `r rawpar$weeks_scale_res`$ weeks; these are the same as the reserved-capacity times for SSV.


Facility transition occurs in week 1. Thus manufacturing begins in week $1+I_R = `r 1+rawpar$weeks_init_res`$ and dose distribution begins in week $1+W_{3; 365}^{(S)} = `r 1+rawpar$weeks_P3_365`$.
