...
demand65 = pall$final_vaccine_coverage*pops365 # total bpsv doses per income level

demand15 = pall$final_vaccine_coverage*pops3*2
...
pto_bpsv_phase3 = pall$n_bpsv_candidates*pto[,3]
...
ssv_rd_costlist = lapply(1:length(dms),function(x) t(apply(inex_costs,1, function (r) r*timescale[x,]/timescale[1,])))

# add licence cost column (adjusted for inflation)
ssv_rd_costsamples = lapply( 1:length(ssv_rd_costlist), function(x) cbind( rowSums( cbind(ssv_rd_costlist[[x]], (1+pall$inflation)*pall$cost_lic)*pto), paste0(dms[x], ' Days Mission')) )

## bpsv reactive r&d costs

bpsvresrd = pto_bpsv_phase3 * (pall$duration_3/phase_duration[1,4] * inex_costs[,4] + pos[,4]*(1+pall$inflation)*pall$cost_lic)
...
and the cost of each phase is $T_i$, a weighted average of experienced and inexperienced manufacturers (assuming $\omega = `r pall$inex_weight`$):
...
The cost per dose per year is $A_1 = `r pall$cost_bpsvinv`$ USD. Then the cost to maintain the reserve of 100,000 doses is
...
```{r bpsvinv,fig.cap='BPSV investigational reserve costs accumulated from year 6 to year 15 with uniformly distributed discount rate.'}
...
inv_cost = pall$cost_bpsvinv*(fifteen_yr_discount_weight-five_yr_discount_weight) *100000/1e6
...
for(i in unique(ssv_rd_plot$Investment)){
  subs = subset(ssv_rd_plot, Investment==i)
  cat(i)
  cat('\n')
  print(round(summary(subs$Cost/1e9*pall$n_ssv_candidates*at_yr_15),2))
  cat('\n')
}
...
```{r}
eqtext = 'D_s^{\\text{(BP-resRD)}} = \\left\\{\\begin{array}{lr}N^{\\text{(BPSV)}}\\hat{P}_3\\left(\\frac{Y_3^{(B)}}{W_{3;365}^{(S)}}\\left(\\omega T_3^{(n)} + (1-\\omega)T_3^{(e)}\\right) + (1+I) P_3L\\right) \\; & \\; s\\in\\{1,2,3\\} \\\\
0  \\; & \\; s\\notin\\{1,2,3\\}
\\end{array}\\right.
(\\#eq:bpsvresrd)'
if (knitr::is_html_output()) {
...
print(round(summary(bpsvresrd/1e6*at_yr_15)))
...
The cost per dose comes from the cost of goods supplied ($G = `r pall$cost_bpsv`$) adjusted for profits ($M_p = `r pall$profit`$) and the fill/finish cost ($M_f = `r pall$cost_ff`$).

$G(1+M_p)(1+M_f)$ evaluates to `r round(pall$cost_bpsv*(1+pall$profit)*(1+pall$cost_ff),3)`, which is slightly more than the given cost per reserved dose, $S_R = `r pall$cost_res`$.

This cost is used both for SSV doses manufactured using reserved capacity, and all BPSV doses.
...
Here, $S_R = `r pall$cost_res`$ is the cost per reserved dose and $S_U = `r pall$cost_un`$ the cost per unreserved dose in USD. 
...
for(i in 1:length(scen_dm)){
  ...
  man_cap_res = (pall$hic_cap_res+crs)[scen_capres[i]]
...
for(s in 1:length(scennames)){
  ...
  cost_per_year[[s]] = c(sapply(1:ceiling(nweeks/52), function(y) 
    min(pall$man_curr,annual_doses[y])*pall$cost_res #*(1+pall$profit)*(1+pall$cost_ff) 
    + max(annual_doses[y]-pall$man_curr, 0)*pall$cost_un
    ), scennames[s])
}
...
bpsvproc = pall$cost_res*sum(demand65)/1e9

```

For a world population aged 65 and over of `r round(sum(pops365)/1e9,1)` billion, an uptake of 80\%, and a cost per dose of $S_R = `r pall$cost_res`$ USD (the same as for SSV via reserved capacity), the procurement cost for BPSV is `r round(bpsvproc,2)` billion USD.
...
For populations aged 15 and above $N_i^{(15)}$ in income group $i\in\{\text{LIC, LMIC, UMIC, HIC}\}$, we have delivery cost:
...
print(summary(unlist(ssv_delivery_costs)))
...
```{r bpsvdelivery,fig.cap="BPSV delivery cost"}
...
```{r}
eqtext = 'Z_{x,s,w} = \\left\\{\\begin{array}{lr}0 & w-w_s^{(0)} < I_x \\\\ 
...
Denote the weekly allocated doses at week $w$ from capacity $x$ to income level $i$ $k_{s,x,i,w}$, and the cumulative number $K_{s,i,w}$, such that $$K_{s,i,w} = \sum_{x\in\\{R,E,B\\}}\sum_{j=0}^w k_{s,x,i,j}.$$ We write $X_i = 2\cdot \lambda\cdot N_i^{(15)}$ as the maximum demand for income group $i$, representing two doses each for $\lambda=80$\% of the population.
...
The duration of the Phase three trial is 18 weeks. **The time to manufacturing transition is 12 weeks, and the time to manufacturing scale-up 10 weeks; these are the same as the reserved-capacity times for SSV. Are these reservations included in the preparedness cost?**
