...
Here, $S_R = `r pall$cost_res`$ is the cost per reserved dose and $S_U = `r pall$cost_un`$ the cost per unreserved dose in USD. Reserved doses are marked up by $M_p = `r pall$profit`$ and $M_f = `r pall$cost_ff`$. 
...
For a world population aged 65 and over of `r round(sum(pops365)/1e9,1)` billion, an uptake of 80\%, and a cost per dose of $G = `r pall$cost_bpsv`$ USD, the procurement cost for BPSV is `r round(bpsvproc,2)` billion USD.
