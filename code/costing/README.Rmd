...
bpsv_upfront = pardf$bpsv_res_upfront*pfixed$bpsv_inv_res
...
for(s in 1:nscen){
  ...
  for(i in 1:NSAMPLES){
    ...
    ssv_proccost_undiscounted[[s]][i] = sum(ssv_procurement_costs)
  }
...
ssv_res_diff = lapply(c(1,5,6), function(x)ssv_proccost_undiscounted[[x]] - ssv_proccost_undiscounted[[1]])
...
for(s in 1:length(scennames)){
  ...
  if(bpsv_scen[s]==T) total_costs <- total_costs + bpsvresrd/1e3 + bpsvproc + bpsv_del_cost/1e3
  
  whichcap = scen_capres[s]
  total_costs <- total_costs + ssv_proccost_undiscounted[[s]] - ssv_proccost_undiscounted[[1]]
