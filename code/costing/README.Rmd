...
$$T_{i} = \omega T_i^{(n)} + (1-\omega)T_i^{(e)}.$$ Then the total cost for phases 0 through 2 is
...
This is the cost per candidate. We multiply by the number of candidate, $N^{SSV}=18$, to get the total cost.
...
The BPSV has $N^{BPSV}=8$ candidates. Those that have passed through Phases 0 to 2 prior to the outbreak go through Phase 3 during the response. The duration is 18 weeks. Thus we write the BPSV R\&D response cost
