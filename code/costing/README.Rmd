...
if (knitr::is_html_output()) {
  # cat(paste0("```math\n",eqtext,"\n```",collapse=''))
  caps$Source <- gsub('}','',caps$Source)
  caps$Source <- gsub('\\\\citet\\{','@',caps$Source)
} else {
  # cat(paste0("$$\n",eqtext,"\n$$",collapse=''))
}
...
and the cost of each phase is $T_i$, a weighted average of experienced and inexperienced manufacturers (assuming $\omega=0.5$):
...
if (knitr::is_html_output()) {
  ...
  # cat(paste0("\\begin{equation}\n",gsub("\\\\(?=[[:punct:]](?![[:space:]]))", "\\\\\\\\", eqtext, perl = TRUE),"\n\\end{equation}\n",collapse=''))
} else {
  cat(paste0("\\begin{equation}\n",eqtext,"\\end{equation}\n",collapse=''))
}
...
```{r}
eqtext = 'D_s^{\\text{(BP-inv)}} = \\left\\{\\begin{array}{lr}
100000 A_1
\\; & \\; s\\in\\{1,2,3\\} \\\\
0  \\; & \\; s\\notin\\{1,2,3\\}
\\end{array}\\right.
...
```{r}
eqtext = 'M_{R,s} = \\left\\{\\begin{array}{lr}0.5 & s\\in\\{0, 1, 6, 9, 12\\} \\\\ 
1.2 & s\\in\\{2, 4, 7, 10\\} \\\\ 
...
Trial costs are adjusted for the duration of the trial, which depend on the R\&D investment, denoted $x\in\{365, 200, 100\}$: $$T_{x,i} = \frac{W_{i;x}^{(S)}}{W_{i;365}^{(S)}}T_i.$$ Then the total cost is
...
```{r}
eqtext = 'x(s) = \\left\\{\\begin{array}{lr} 365 & s\\in\\{0, 1, 2, 3, 4, 5, 12\\} \\\\ 
200 & s\\in\\{6, 7, 8\\} \\\\ 
...
We multiply by the number of candidate, $N^{\text{(SSV)}}=18$, to get the total cost from the weighted average per candidate.
...
The BPSV has $N^{\text{(BPSV)}}=8$ candidates. Those that have passed through Phases 0 to 2 prior to the outbreak go through Phase 3 during the response. The duration is 18 weeks. Thus we write the BPSV R\&D response cost
