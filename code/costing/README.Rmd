...
1. On sheet "6. SARS-X R&D" there are two "SARS-X Preclin PoS", and no "Preclin Duration". The "Preclinical / Trad Weeks" comes from Phase 1.  
2. Cell C18 on sheet "11. Delivery" -> 10%
3. Stop first SSV doses at half the number of allocated doses. This changes delivery costs slightly when deliveries stop just after a change of year, due to discounting. It should not affect delivery in the impact model. 
4. In "12. Cashflow & PV", it looks like Advance Cashflows discount to 2025, and Response Cashflows to 2024. We could correct the latter to 2025, or, better still, change both to 2026.
5. Why do booster doses start again in terms of cost per dose by phase for delivery? Can I suggest the cost for delivering these doses should be the third phase cost?

Clarifications

1. 14 BPSV candidates or 8?
2. Do all BPSV start at Phase 0, or do some start at Phase 1?
3. BPSV R&D costs appear to be inflated for inflation (28%) for BPSV prep R&D but not licensure and not BPSV Phase 3. For SSV, phase costs are inflated but licensure isn't.
4. What is the stockpile upfront cost?
5. BPSV response R&D does not appear to depend on number of candidates, probability of success, probability to occur, or inflation. Should it?
6. SSV PoS uses a beta distribution and refers to COVID data (sheet "PoS by Phase"). The formula to describe a beta distribution for probability p using data is p~Beta(a, b), where a=successes+1 and b=failures+1. So for preclinical we have p~Beta(97, 34). And in the final phase, p~Beta(28, 9), because there were 35 remaining candidates, of which 27 passed and 8 failed. Did I understand this table correctly? And is there a source for these data?

Nice to have?

1. Why was 1000000000 (1 billion) chosen as the target amount for BPSV production? This can be rationalised by setting vaccine wastage to 0.3142532. (NB: no vaccine wastage is assumed for SSV.)
2. Some values in "1. Input Dashboard" are calculated rather than given (e.g. cost for the capacity reservation, 160000000*1.08/325000000;  BSPV investigational cost per dose, 12145636/1200000000; the stockpile upfront cost, 138000000/1200000000 per dose). Can we include the sources & rationale for these values?
...
and the cost of each phase is $T_i$, a weighted average of experienced and inexperienced manufacturers (with $\omega = `r rawpar$inex_weight`$):
...
where $A_1 = `r format_to_print(rawpar$cost_bpsv_res,0)`$ USD is the annual reservation cost.
...
The cost per dose reservation per year is $A_2 = `r rawpar$cost_capres`$ USD. Reservation sizes, in billions, depend on scenarios, including the $A_3 = `r rawpar$hic_cap_res`$ billion doses reserved for HIC, as follows:
...
The annual costs in billion USD are `r 0.5*rawpar$cost_capres`, `r 1.2*rawpar$cost_capres`, and `r 2.5*rawpar$cost_capres`, respectively.
...
for $i \in \left\{0,1,2,3 \right\}$ where $X_i$ is the number of candidates that failed in phase $i$ and $X_4$ the number that succeeded.
...
Here, $S_R = `r round(cost_res,2)`$ is the cost per reserved dose and $S_U = `r rawpar$cost_un`$ the cost per unreserved dose in USD. 
...
For a world population aged 65 and over of `r round(sum(POPS65)/1e9,1)` billion, an uptake of 80\% (accounting for wastage of `r round(100*rawpar$vaccine_wastage)`\%), and a cost per dose of $S_R = `r round(cost_res,2)`$ USD (the same as for SSV via reserved capacity), the procurement cost for BPSV is `r round(bpsvproc[1]/1e3,2)` billion USD.
...
bpsv_rd_prep = as.data.frame(t(quantile(bpsv_rd_costsamples_no_d/1e3,c(1:3)/4)))
...
# enabling_costs
enab = as.data.frame(t(sapply(c(5,15),function(x) quantile(rawpar$cost_enab*x,c(1:3)/4)/1e3)))
enab$timing = 'One-off'
enab$category = 'R&D'
enab$type = missions
...
bpsv_man = as.data.frame(t(quantile(bpsv_upfront/1e3,c(1:3)/4)))
bpsv_man$timing = 'One-off'
...
# capres_costs
ssv_capres = as.data.frame(t(sapply(crs[2:3],function(x) quantile(x*rawpar$cost_capres,c(1:3)/4))))
...
# bpsvresrd
bpsv_rd_res = as.data.frame(t(quantile(bpsvresrd/1e3,c(1:3)/4)))
...
ssv_res_diff = lapply(c(5,6), function(x)ssv_proccost_undiscounted[[x]] - bau_ref)
...
bpsv_proc_res = as.data.frame(t(quantile(bpsvproc/1e3,c(1:3)/4)))
...
ssv_del_diff = lapply(c(5,6,13),function(x)(ssv_delivery_costs[[x]]-bau_del)/1e3)
...
bpsv_del = as.data.frame(t(quantile(bpsv_del_cost/1e3,c(1:3)/4)))
...
# crs in billions
bpsv_ss = as.data.frame(t(sapply(1:length(crs),function(x) quantile(bpsv_rd_costsamples_dyd + inv_cost/1e3 + crs[x] *rawpar$cost_capres*fifteen_yr_discount_weight,c(1:3)/4))))
...
capres_ss = as.data.frame(t(sapply(1:length(crs),function(x) quantile( crs[x] *rawpar$cost_capres*fifteen_yr_discount_weight,c(1:3)/4))))
...
dm200_ss = as.data.frame(t(sapply(1:length(crs),function(x) quantile( crs[x] *rawpar$cost_capres*fifteen_yr_discount_weight + enabling_costs_list[[2]]/1e3,c(1:3)/4))))
...
dm100_ss = as.data.frame(t(sapply(1:length(crs),function(x) quantile( crs[x] *rawpar$cost_capres*fifteen_yr_discount_weight + enabling_costs_list[[3]]/1e3,c(1:3)/4))))
...
for(s in 1:length(scennames)){
  all_prep_costs[[s]] = quantile(cost_diffs[[1]][,s]+cost_diffs[[2]][,s]*fifteen_yr_discount_weight,c(1:3)/4)/1e3
  all_resp_costs[[s]] = quantile(cost_diffs[[3]][,s]*at_yr_15,c(1:3)/4)/1e3
}

preps = as.data.frame(do.call(rbind,all_prep_costs))
preps$Scenario = scennames
preps$Category = c('365 days', rep('BPSV',3), rep('365 days',2),rep(c('200 days','100 days'),each=3), 'Equality + Delivery')
...
fifcosts = ggplot(toplot) +
  ...
  geom_text(aes(x=`Preparedness_50%`+.8,y=`Response_50%`+2.5*(1-2*as.numeric(Scenario=='S12')),label=Scenario)) +
...
ggplot(subset(t_supply,scen%in%scennames[2:4])) + 
  geom_line(aes(x=as.numeric(week),y=as.numeric(doses),colour=factor(scen,levels=scennames))) +
  theme_bw(base_size = 15) +
  labs(x='Week',y='Cumulative doses manufactured, billions',colour='Scenario') +
...
as the maximum demand for income group $i$, representing two doses each for $\lambda=80$\% of the population assuming vaccine wastage of $\delta = `r rawpar$vaccine_wastage`$.
