...
```{r}
eqtext = 'D_{s,y}^{\\text{(BP-proc)}} = \\begin{cases}
...
We write the delivery cost, $H_{s,i,w}$, for $h_{s,i,w}$ doses given in week $w$ and income level $i$ as follows. There are three cost tiers, the first of which, $V_{i; 0}$, is applied to the first 10\% of $L_i$, the second ($V_{i; 11}$) to the subsequent 20\%, and the third ($V_{i; 31}$) to all doses thereafter. The same costing schedule applies both to the first-schedule plus booster SSV doses and the BPSV rollout. 
