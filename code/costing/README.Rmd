...
library(wpp2024)
income_levels = c("Lower-middle-income countries","Low-income countries","Upper-middle-income countries","High-income countries")
data(popMprojMed,package = 'wpp2024')
data(popFprojMed,package = 'wpp2024')
subset(popMprojMed, name%in%income_levels)[,c(2,3,4)] -> xm
subset(popFprojMed, name%in%income_levels)[,c(2,3,4)] -> xf
xm$m = 'M'
xf$m = 'F'
setDT(rbind(xm,xf)) -> alldata
colnames(alldata) <- paste0('x',colnames(alldata))

summeddata = alldata[,sum(x2025),by=.(xname,xage)]
summeddata[,lowerage:=as.numeric(strsplit(xage,'[[:punct:]]')[[1]][1]),by=xage]
summeddata[,over64:=ifelse(lowerage>64,V1,0)]
summeddata[,over14:=ifelse(lowerage>14,V1,0)]
summeddata[,lapply(.SD, sum),by=xname,.SDcols=c('V1','over14','over64')]
...
caps <- readODS::read_ods('cost_parameters.ods')
...
# 
# 1 - rawpar$final_vaccine_coverage*sum(pops65)/1e9
...
- $D_{s,y}^{\text{(BP-sec)}}$ is the upfront cost of securing an investigational reserve of 100,000 BPSV doses; see Equation \@ref(eq:bpsvadman)
...
In order to achieve one successful BPSV candidate in the event of an outbreak, we estimate that 14 are required to enter into preclinical R\&D. At present, there are six (Table \@ref(tab:inex)). We assume that the remaining eight will come from developers without prior licensure experience.
...
```{r}
eqtext = '\\hat{P}_i^{(0)} = \\begin{cases}1 & i=0 \\\\ 
...
The cost of each phase is $T_i$, a weighted average of experienced and inexperienced manufacturers. Assuming that $N^{\text{(BPSV)}}-N^{\text{(BPSV-1)}}=`r rawpar$n_bpsv - rawpar$n_bpsv_p1`$ candidates start in the preclinical phase, and only one so far has experience with licensure (Table \@ref(tab:inex)), we take $\omega = `r round(rawpar$inex_weight,2)`$;
...
where $\pi = `r rawpar$inflation`$ is inflation from 2018 to 2025. Then the total weighted cost for phases 0 through 2 for $N^{\text{(BPSV)}}$ candidates is
...
# sum(rbinom(nrow(PTO_BPSV),14,PTO_BPSV[,3])>=1)/nrow(PTO_BPSV)*100
# summary(1-pbinom(0,size=14,prob=PTO_BPSV[,3]))
# pbinom(0,size=14,prob=PTO_BPSV[349,3])
...
Denoting the duration of each Phase $i$ as $Y_i^{(B)}$, the time taken to complete development of the BPSV up to the end of Phase 2, from which point it is manufactured to be held in an investigational reserve, is:
...
where $A_4 =`r format_to_print(rawpar$bpsv_inv_res)`$ is the size of the reserve and $A_5 =`r format_to_print2(rawpar$bpsv_res_upfront,3)`$ is the cost per dose in USD, coming from the upfront cold-chain cost estimate of 138 million USD for 1.2 billion doses [@Griffiths2021a].

The cost of goods supplied is $G = `r rawpar$cost_cogs`$ USD per dose. Then the cost of drug substance, accounting for the fill/finish cost $M_f = `r round(rawpar$cost_ff,2)`$ and the profit margin $M_p = `r round(rawpar$profit,2)`$, is $G(1-M_f)(1+M_p) = `r round(rawpar$cost_cogs*(1-rawpar$cost_ff)*(1+rawpar$profit),2)`$ USD per dose. We assume that the reserve is replenished every $Y_{rep} = `r rawpar$bpsv_replenishment`$ years based on the storage characteristics for Ervebo [@WorldHealthOrganization2019a]. 

Then the annual cost to maintain the reserve of $A_4 =`r format_to_print(rawpar$bpsv_inv_res)`$ doses is
...
ggplot(capres_costs) +
  geom_boxplot(aes(x=factor(V2,levels=crs, labels=crs+.5),y=V1)) +
  theme_bw(base_size = 15) +
...
One BPSV candidate that has passed through phases 0 to 2 prior to the outbreak goes through Phase 3 during the response. The duration is $W_3^{(B)}=`r rawpar$duration_3_resp`$ weeks. Thus we write the BPSV R\&D response cost


```{r}
eqtext = 'D_s^{\\text{(BP-resRD)}} = \\begin{cases}\\left( (1+\\pi)\\frac{W_3^{(B)}}{52Y_3^{(B)}}T_3^{(e)} + P_3 T_4\\right) \\; & \\; s=1 \\\\
0  \\; & \\; s\\neq 1
\\end{cases}
(\\#eq:bpsvresrd)'
printeq(eqtext)
...
The cost per dose comes from the cost of goods supplied, $G = `r rawpar$cost_cogs`$ USD, adjusted for profits ($M_p = `r rawpar$profit`$) and the transportation cost ($M_t = `r rawpar$cost_travel`$).
...
For a world population aged 65 and over of `r round(sum(POPS65)/1e9,2)` billion, an uptake of `r 100*rawpar$final_vaccine_coverage`\% (accounting for wastage of `r round(100*rawpar$vaccine_wastage)`\%), and a cost per dose of $S_R = `r round(cost_res,2)`$ USD (the same as for SSV via reserved capacity), the procurement cost for BPSV is `r round(bpsvproc[1]/1e3,2)` billion USD.
...
as the total demand for first-schedule doses for income group $i\in\lbrace\text{LIC, LMIC, UMIC, HIC}\rbrace$, representing two doses each for $\lambda = `r rawpar$final_vaccine_coverage*100`$\% of the population.

We write the delivery cost, $H_{s,i,w}$, for $h_{s,i,w}$ doses given in week $w$ and income level $i$ as follows. There are three cost tiers, the first of which, $V_{i; 0}$, is applied to the first 10\% of $L_i$ (the "start up" cost); the second ($V_{i; 11}$) to the subsequent 20\% (the "ramp up" cost); and the third ($V_{i; 31}$) to all doses thereafter (the "at scale" cost). The same costing schedule applies both to the first-schedule plus booster SSV doses and the BPSV rollout. 
...
fifcosts = ggplot(toplot) +
  ...
  labs(linetype='',x='Preparedness costs, billion USD',y='Response costs, billion USD',colour='',size='Capacity\nreservation',
...
```{r}
eqtext = 'w_s^{(0)} = W_{\\zeta(s)}^{(S)} - 7 = \\begin{cases}45 & s\\in\\lbrace 0, 1, 2, 3, 10\\rbrace \\\\ 
...
The maximum total global manufacturing volume per year is $M_G=15$ billion doses. The volume that is reserved, in billion doses, including the HIC-specific reservation of $A_3=0.5$ billion doses, depends on the scenarios as follows:
...
The duration of the Phase 3 trial is $W_3^{(B)} = `r rawpar$duration_3_res`$ weeks. The time to manufacturing transition is $I_R = `r rawpar$weeks_init_res`$ weeks, and the time to manufacturing scale-up $C_R = `r rawpar$weeks_scale_res`$ weeks; these are the same as the reserved-capacity times for SSV.
