...
fifteen_yr_discount_weight = rowSums(sapply(1:15,function(x)1/(1+pall$discount)^x))
five_yr_discount_weight = rowSums(sapply(1:5,function(x)1/(1+pall$discount)^x))
...
bpsvrd = pto_bpsv_phase3 * (pall$duration_3/phase_duration[1,4] * inex_costs[,4] + pos[,4]*(1+pall$inflation)*pall$cost_lic)
...
```{r}
eqtext = 'D_s^{\\text{(BP-resRD)}} = \\left\\{\\begin{array}{lr}N^{\\text{(BPSV)}}\\hat{P}_3\\left(\\frac{18}{W_{3;365}^{(S)}}\\left(\\omega T_3^{(n)} + (1-\\omega)T_3^{(e)}\\right) + (1+I) P_3L\\right) \\; & \\; s\\in\\{1,2,3\\} \\\\
0  \\; & \\; s\\notin\\{1,2,3\\}
\\end{array}\\right.
(\\#eq:bpsvresrd)'
if (knitr::is_html_output()) {
...
```{r bpsvrd,fig.cap='Reactive R&D cost for BPSV'}

ggplot() + geom_histogram(aes(bpsvrd/1e6)) +
  theme_bw(base_size=15) +
  labs(x='Cost, million USD',y='',title='')

print(round(summary(bpsvrd/1e6*at_yr_15)))
