function Td = get_doubling_time(data,dis)
    ...
    J(1:ntot,2*ntot+1:3*ntot)          = -dis.beta*dis.red*data.basic_contact_matrix;
    J(1:ntot,3*ntot+1:4*ntot)          = -dis.beta*data.basic_contact_matrix;
    J(1:ntot,5*ntot+1:6*ntot)          = diag(onesn.*dis.nu);
    J(ntot+1:2*ntot,1*ntot+1:2*ntot)   = diag(onesn.*(-dis.sig1-dis.sig2));
    J(ntot+1:2*ntot,2*ntot+1:3*ntot)   = dis.beta*dis.red*data.basic_contact_matrix;
    J(ntot+1:2*ntot,3*ntot+1:4*ntot)   = dis.beta*data.basic_contact_matrix;
