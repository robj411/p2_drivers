...
params:
  lbfile: './data/vaccine_delivery.xlsx'
---
...
```{r popdata}
# coverages apply to whole population
caps <- readODS::read_ods('code/costing/cost_parameters.ods',sheet = 1)
populationhic0 = as.numeric(subset(caps,Code=='pop_hic_0')$Parameters)
populationumic0 = as.numeric(subset(caps,Code=='pop_umic_0')$Parameters)
populationllmic0 = sum(as.numeric(subset(caps,Code%in%c('pop_lmic_0','pop_lic_0'))$Parameters))
pops0 = c(populationllmic0, populationumic0, populationhic0)
populationhic15 = as.numeric(subset(caps,Code=='pop_hic_15')$Parameters)
populationumic15 = as.numeric(subset(caps,Code=='pop_umic_15')$Parameters)
populationllmic15 = sum(as.numeric(subset(caps,Code%in%c('pop_lmic_15','pop_lic_15'))$Parameters))
pops15 = c(populationllmic15, populationumic15, populationhic15)
populationhic65 = as.numeric(subset(caps,Code=='pop_hic_65')$Parameters)
populationumic65 = as.numeric(subset(caps,Code=='pop_umic_65')$Parameters)
populationllmic65 = sum(as.numeric(subset(caps,Code%in%c('pop_lmic_65','pop_lic_65'))$Parameters))
pops65 = c(populationllmic65, populationumic65, populationhic65)
# pops0/sum(pops0)
# pops15/sum(pops15)
```
...
get_to_print = function(qu, flag=0){
  if(flag==1)
    if(qu[2]<1&qu[1]>0)
      return('<1')
  pq = format_to_print2(qu)
  if(pq[1]==pq[2]){
    pq[1]
  }else{
    paste0(pq, collapse='--\\hspace{0pt}')
  }
}
quants = c(1,3)/4
...
for(s in 1:nScen){
  ...
  iqrs[[s]]$bpsvrdpc = (with(costtab, get_to_print(quantile(100*bpsv_response_rd/totalundiscounted,quants),flag=1)))
  iqrs[[s]]$bpsvprocpc = (with(costtab, get_to_print(quantile(100*bpsv_proc/totalundiscounted,quants),flag=1)))
  iqrs[[s]]$bpsvdelpc = (with(costtab, get_to_print(quantile(100*bpsv_delivery/totalundiscounted,quants),flag=1)))
  iqrs[[s]]$ssvrdpc = (with(costtab, get_to_print(quantile(100*ssv_rd/totalundiscounted,quants),flag=1)))
  iqrs[[s]]$deliverypc = (with(costtab, get_to_print(quantile(100*ssv_delivery_undiscounted/totalundiscounted,quants),flag=1)))
  iqrs[[s]]$procpc = (with(costtab, get_to_print(quantile(100*ssv_proc_undiscounted/totalundiscounted,quants),flag=1)))
}
...
newrownames = c(enabling='Enabling activities',
                ...
                bpsvrdpc='BPSV response R\\&D',
                bpsvprocpc='BPSV response procurement',
                bpsvdelpc='BPSV delivery',
                ssvrdpc='SSV R\\&D',
                deliverypc='SSV delivery',
                procpc='SSV procurement')
...
```{r gini,message=F,echo=F,warning=F,fig.cap='Gini coefficient over time for SSV distribution.',fig.height=4}
...
plotgini <- do.call(rbind,df_list)
plotgini$Week = plotgini$Day/7
setDT(plotgini)

ginifunction = function(percents){
  x = cumsum(pops15/sum(pops15))
  ye = x
  yl = cumsum(percents/100*pops0) / sum(percents/100*pops0)
  # area under lorenz curve
  B = x[1]*yl[1]/2 + (yl[1]+yl[2])/2 * (x[2]-x[1]) + (yl[2]+yl[3])/2 * (x[3]-x[2])
  # area under equality curve
  C = x[1]*ye[1]/2 + (ye[1]+ye[2])/2 * (x[2]-x[1]) + (ye[2]+ye[3])/2 * (x[3]-x[2])
  # area between line of equality and lorenz curve
  A = C - B
  G = A / (A+B)
  max(G,0)
}
plotgini[,Income:=factor(plotgini$Income,levels=c('LLMIC','UMIC','HIC'))]
setorder(plotgini, Income)
plotgini[,Gini:=ginifunction(Percent),by=.(Day,Scenario)]

percents = subset(plotgini, Day==896&Scenario=='S10')$Percent
# max(subset(plotgini,!is.na(Gini))$Gini)
# subset(plotgini,!is.na(Gini)&Gini<0.045&Day==896)

pl1 = ggplot(subset(plotgini,!Scenario%in%c('BAU2')&Week<200)) + 
  geom_line(aes(x=Week,y=Gini,colour=factor(Scenario,levels=scenario_names[c(1:11)])),linewidth=1,alpha=1) +
  theme_bw(base_size=15) + labs(colour='') +
  theme(legend.position = 'top') + 
  guides(colour=guide_legend(nrow=2,byrow=F)) +
  scale_y_continuous(limits = c(0,1)) +
  scale_x_continuous(expand = c(0,0)) +
  scale_colour_manual(values = with(subset(plotgini, Scenario != "BAU2"), {
...
bpsv_cutoffs = rev(80*pops65/pops0)
ssv_cutoffs = rev(80*pops15/pops0)
...
for(scen in 1:nScen){
  for(il in 1:3){
    ...
    for(i in c(0,3)){
      vaccine <- tab[1,start_index+i][[1]]
      vindex <- get_index + i
      pcval <- pcval +  cumsum(as.numeric(tab[-c(1:2),vindex][[1]]))*100
      if(i==0) {
        pcval = pmin(pcval, bpsv_cutoffs[il])
        total_cutoff = max(pcval) + ssv_cutoffs[il]
      }
    }
    max80 = which(pcval>total_cutoff)[1]
    pcval[(max80+1):length(pcval)] = pcval[max80+1]
    df_list[[length(df_list)+1]] <- data.frame(Day=1:length(pcval), Percent=pcval, #Vaccine=vaccine,
                                               Scenario=scenario, Income=ilevel)
  }
}
...
pl1 = ggplot(subset(plotscens,!Scenario%in%c('BAU2')&Week<200)) + 
  geom_line(aes(x=Week,y=Percent,colour=factor(Scenario,levels=scenario_names[c(2:11,1)])),linewidth=.5,alpha=1) +
...
```{r,eval=F}
plot2 = setDT(subset(plotscens,Scenario%in%c('BAU','S10')))
plot2[,maxic:=max(Percent),by=.(Income,Scenario)]
plot2[,minmaxic:=min(maxic),by=.(Income)]
plot2[,Percent:=min(Percent,minmaxic),by=.(Day,Income,Scenario)]
ggplot(plot2) + 
  geom_line(aes(x=Week,y=Percent,colour=factor(Scenario,levels=scenario_names[c(1,11)],labels=c('Business as usual','Equality + Delivery'))),linewidth=1,alpha=1) +
  facet_grid(~factor(Income,levels=c('LLMIC','UMIC','HIC')),scales='free_y') + 
  theme_bw(base_size=15) + labs(colour='',y='Vaccine coverage, %') +
  theme(legend.position = 'top') + 
  guides(colour=guide_legend(nrow=1,byrow=F))
ggplot(subset(plot2,Scenario=='BAU')) + 
  geom_line(aes(x=Week,y=Percent,colour=Income),linewidth=1,alpha=1) +
  theme_bw(base_size=15) + labs(colour='',y='Vaccine coverage, %') +
  theme(legend.position = 'top') + 
  guides(colour=guide_legend(nrow=1,byrow=F))
...
```{r violintop,fig.cap='Model results for the cost-minimising responses.',echo=F,warning=F,message=F,fig.height=5,fig.width=6,eval=F}
...
```{r violinBAU,fig.cap='Model results for the 365-day SARS-X vaccine without the BPSV.',echo=F,warning=F,message=F,fig.height=5,fig.width=6,eval=F}
p[[scen_to_keep[1]]]
...
```{r violinscen1,fig.cap='Model results for the 100-day SARS-X vaccine without the BPSV.',echo=F,warning=F,message=F,fig.height=5,fig.width=6,eval=F}
p[[scen_to_keep[2]]]
```

```{r violinscen2,fig.cap='Model results for the 365-day SARS-X vaccine with the BPSV.',echo=F,warning=F,message=F,fig.height=5,fig.width=6,eval=F}
p[[scen_to_keep[3]]]
```

```{r violinscen3,fig.cap='Model results for the 100-day SARS-X vaccine with the BPSV.',echo=F,warning=F,message=F,fig.height=5,fig.width=6,eval=F}
p[[scen_to_keep[4]]]
```

```{r violinscen4,fig.cap='Model results for the 100-day SARS-X vaccine with the BPSV.',echo=F,warning=F,message=F,fig.height=5,fig.width=6,eval=F}
p[[scen_to_keep[5]]]
```
...
```{r expectedvaluesbar,fig.cap='Expected values of model results.',echo=F,warning=F,message=F,fig.height=6,fig.width=6,eval=F}
...
```{r diffviolin,fig.cap='Model results for losses saved by 100-day and BPS vaccines.',echo=F,warning=F,message=F,fig.height=4,fig.width=6,eval=F}
...
# write.csv(tabres,'results/Delta_LIR_IQR_IL.csv',row.names = F, quote = F)
...
```{r ex30bar,fig.cap='Distributions of values of investments for once-in-thirty-years events (top) and once-in-one-hundred-years events (bottom).',echo=F,warning=F,message=F,fig.height=8,fig.width=10}
plotlist
```
...
accumulate = function(lq, uq, start_year=2050, end_year=2099, start_gdp=113.8, start_gdp_year=2025, discount=0, growth=0, pv_year=2025){
  ...
  lqv = lq*pv/100 # quartiles are percentages
  uqv = uq*pv/100
