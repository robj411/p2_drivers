...
```{r voi,fig.cap='Value of information for pre-alpha variants of SARS-CoV-2.',echo=F,warning=F,message=F,fig.height=10}

voiall <- readRDS('results/voi.Rds')

nscen <- nrow(voiall)
nparam <- ncol(voiall)
brks <- c(-1,10,30,50,70,101)
...
lbls <- c('<10','10-30','30-50','50-70','70-100')
ggplot(df, aes(x, y, fill = factor(xs,levels=1:(length(brks)-1),labels=lbls))) +
  geom_tile() +
...
```{r mi,fig.cap='Because of the very heavily skewed distribution of the food and accommodation services sector as a share of GDP, mutual information (MI) might be better.',echo=F,warning=F,message=F,fig.height=10}

voiall <- readRDS('results/mi.Rds')

nscen <- nrow(voiall)
nparam <- ncol(voiall)
brks <- c(-1,10,20,40,60,90)/100
discxs <- matrix(cut(as.numeric(voiall),breaks=brks,labels=1:(length(brks)-1)),
                 nrow=nscen,ncol=nparam)
df <- data.frame(x=rep(xvals,each=nscen),y=rep(yvals,nparam),xs=c(discxs[nscen:1,]))

options(repr.plot.width = 1, repr.plot.height = 0.75)
lbls <- c('<.1','.1-.2','.2-.4','.4-.6','.6-.9')
ggplot(df, aes(x, y, fill = factor(xs,levels=1:(length(brks)-1),labels=lbls))) +
  geom_tile() +
  scale_fill_manual(values = colorRampPalette(c("midnightblue","royalblue","lightskyblue","white"))(length(brks)-1)) +   
  theme_bw(base_size=12) +                                   # Minimal theme
...
```{r Influenza2009,fig.cap='Model results for Influenza 2009.',echo=F,warning=F,message=F,eval=F}
...
```{r Influenza1957,fig.cap='Model results for Influenza 1957.',echo=F,warning=F,message=F,eval=F}
...
```{r Influenza1918,fig.cap='Model results for Influenza 1918.',echo=F,warning=F,message=F,eval=F}
...
```{r omicron,fig.cap='Model results for SARS-CoV-2 omicron.',echo=F,warning=F,message=F,eval=F}
...
```{r delta,fig.cap='Model results for SARS-CoV-2 delta.',echo=F,warning=F,message=F,eval=F}
...
```{r sars,fig.cap='Model results for SARS-CoV-1.',echo=F,warning=F,message=F,eval=F}
...
data <- readODS::read.ods('tourism.ods')[[1]]
...
ytd <- readODS::read.ods('tourism.ods')[[2]]
...
labs <- setDT(read_dta('../Programs/Data/lab_share_data.dta'))
