...
```{r dvoi,fig.cap='Decision VOI. Which parameters would enable the decision maker to choose a strategy that minimises expected losses?',eval=T,echo=F,warning=F,message=F,fig.height=12}

voiall <- readRDS('code/results/decisionvoi.Rds')*100
rownames(voiall) <- gsub('Deaths:','YLLs:',rownames(voiall))
rownames(voiall) <- gsub('School:','Education:',rownames(voiall))
...
```{r mi,fig.cap='Mutual information (MI) for pre-Alpha variants of SARS-CoV-2.',eval=T,echo=F,warning=F,message=F,fig.height=12}
